...
@pytest.mark.parametrize(
    "outcome", ["relocated", "already_relocated", "backup_failed", "relocation_failed", "legacy"]
)
def test_rollback_quarantines_require_complete_backup_before_retention(tmp_path, outcome):
    checkpoints, releases = _roots(tmp_path, range(100, 104))
    operations = tmp_path / "operations"
    operations.mkdir(mode=0o700)
    store = OperationStore(operations, releases)
    ...
    rollback["version"] = 1 if outcome == "legacy" else 2
    for index, record in enumerate(chain):
        _workspace(checkpoints, record["operation_id"])
        store.write(record)
        os.utime(operations / f"{record['operation_id']}.json", (NOW + index, NOW + index))
    runtime = _fake_runtime(tmp_path, releases, checkpoints, live=103, current=103)
    calls = []

    def backup():
        calls.append("backup")
        if outcome == "backup_failed":
            raise SupervisorError("uncaptured project")

    def filesystem(action, request):
        assert request == rollback["checkpoint"]
        if action == "quarantine-status":
            return {
                "version": 1,
                "status": "relocated" if outcome == "already_relocated" else "pending",
                "legacy_quarantines": ["legacy-tree"],
            }
        assert action == "relocate-quarantines"
        calls.append("relocate")
        if outcome == "relocation_failed":
            raise SupervisorError("interrupted relocation")
        return {"version": 1, "status": "relocated", "legacy_quarantines": ["legacy-tree"]}

    runtime.protected_backup = backup
    runtime.filesystem = filesystem
    plan = prune_retained(runtime, store)

    removed = outcome in {"relocated", "already_relocated"}
    assert (checkpoints / rollback["operation_id"]).exists() is not removed
    assert (releases / "100").exists() is not removed
    assert calls == (
        []
        if outcome in {"legacy", "already_relocated"}
        else ["backup"]
        if outcome == "backup_failed"
        else ["backup", "relocate"]
    )
    assert plan.left_alone
