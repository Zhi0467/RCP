...
async def test_delegation_deadline_stops_silent_group_before_typed_failure(
    provider: str, close_stdout: bool, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    hooks = prepare_control(control, [str(tmp_path / "stage")])
    apply_hook("SessionStart", {}, control / "state.json", control / "started")
    monkeypatch.setattr(
        launcher,
        "_prepare_codex_control",
        lambda **_: {
            "hooks": hooks,
            "marker": str(control / "started"),
            "receipt": {"warning_codes": []},
        },
    )
    monkeypatch.setattr("rcp.agents.launcher.DELEGATION_WAIT_LIMIT_SECONDS", 0.1)
    monkeypatch.setattr("rcp.agents.launcher.remaining_startup_hold", lambda _: 0)
    if provider == "claude":
        start = (
            'print(json.dumps({"type":"system","subtype":"task_started","task_id":"child"}), flush=True)\n'
            'print(json.dumps({"type":"result","subtype":"success","result":"held"}), flush=True)\n'
        )
    else:
        start = (
            f"subprocess.run([sys.executable, {str(control / 'hook.py')!r}, 'SubagentStart', "
            f"{str(control / 'state.json')!r}, {str(control / 'started')!r}], "
            "input=json.dumps({'agent_id':'child'}), text=True, check=True)\n"
            f"subprocess.run([sys.executable, {str(control / 'hook.py')!r}, 'Stop', "
            f"{str(control / 'state.json')!r}, {str(control / 'started')!r}], "
            "input='{}', text=True, stdout=subprocess.DEVNULL, check=True)\n"
        )
    script = (
    ...
    monkeypatch.setattr(launcher, "_command", lambda *_, **__: [sys.executable, "-c", script])
    async with asyncio.timeout(5):
        events = [
            event
            async for event in launcher.stream(
                provider, "prompt", cwd=tmp_path, capability="scratch_patch"
            )
        ]
    wait = next(event for event in events if event.event == "delegation_wait")
    assert json.loads(wait.text)["code"] == "open_work"
    exit_index = next(i for i, event in enumerate(events) if event.event == "provider_exit")
    assert json.loads(events[exit_index].text)["verdict"] == "delegation_unfinished"
    assert json.loads(events[exit_index].text)["return_code"] == -signal.SIGKILL
    assert events[exit_index + 1].failure_kind == "delegation_unfinished"
    assert events[-1].event == "error"
    assert not any(event.event == "answer" for event in events)


@pytest.mark.asyncio
async def test_sealed_remote_delegation_failure_survives_ssh_exit_255(
    ...
    script = f"import sys; print('RCP_TURN_RECEIPT:' + {receipt!r}, flush=True); sys.exit(255)"
    monkeypatch.setattr(
        "rcp.agents.launcher.ssh_arguments", lambda *_, **__: [sys.executable, "-c", script]
    )
    stopped = []

    def confirm(host, pid_file, started_at, **kwargs):
        stopped.append((host, pid_file))
        return True
    ...
    events = [
        ...
        async for event in launcher.stream(
            ...
            remote_pid_file="/stage/provider.pid",
            supervisor_path="/stage/supervisor.py",
        )
    ]
    assert stopped == [("execution-host", "/stage/provider.pid")]
    assert not any(event.event == "remote_result_pending" for event in events)
    assert not any(event.event == "remote_process_stop" for event in events)
    exit_index = next(i for i, event in enumerate(events) if event.event == "provider_exit")
    assert json.loads(events[exit_index].text)["verdict"] == "delegation_unfinished"
    assert events[exit_index + 1].failure_kind == "delegation_unfinished"
