...
_FROZEN_ROUTE_INVENTORY: tuple[RouteEntry, ...] = (
    ...
    (("GET",), "/api/projects/{project_id}/compute-jobs"),
    (("POST",), "/api/projects/{project_id}/compute-jobs/{job_id}/cancel"),
...
_HANDLER_MODULE_MAP: dict[str, str] = {
    ...
    "project_compute_jobs": "src/rcp/api/project_state.py",
    "cancel_project_compute_job": "src/rcp/api/project_state.py",
...
def test_frozen_route_inventory(route_app: FastAPI) -> None:
    ...
    assert len(_FROZEN_ROUTE_INVENTORY) == 134
    ...
    assert sum(isinstance(route, APIRoute) for route in routes) == 130
