...
async def test_delegation_failure_retains_patch_before_error(tmp_path, recorded):
    from rcp.agents import AgentEvent

    service, request, execution = _one_result_app(tmp_path)
    patch_text = '{"unfinished": true}'

    class FailedLauncher(ScriptedLauncher):
        async def stream(self, *args, **kwargs):
            async for event in super().stream(*args, **kwargs):
                if event.event == "done":
                    yield AgentEvent(
                        event="error",
                        text="Delegation wait expired.",
                        failure_kind="delegation_unfinished",
                    )
                else:
                    yield event

    launcher = FailedLauncher([{"patch.json": patch_text}])
    if recorded:
        resolved = work_module._resolve_work_execution(service, request, execution)
        turn, staged = await work_module._stage_work_turn(
            service, resolved, tmp_path / "data", execution
        ...
        await turn.validator_lifecycle.close()
        value = _recorded_pass(patch_text, "Not a completed answer")
        ...
        stream = work_module.finalize_recorded_work_result(
            service, launcher, request, tmp_path / "data", execution, value
        )
    else:
        stream = stream_work_run(service, launcher, request, tmp_path / "data", execution)
    errors = []
    async for frame in stream:
        event = AgentEvent.model_validate_json(frame.removeprefix("data: ").strip())
        if event.event == "error":
            assert execution.store.agent_task_patch_output(execution.operation_id) == patch_text
            errors.append(event.failure_kind)
    assert errors == ["delegation_unfinished"]
    assert service.history.state().revision == 1
    assert launcher.calls == (0 if recorded else 1)
    # The next logical turn may clear the stable chat mailbox safely.
...
@pytest.mark.asyncio
async def test_unreachable_delegation_patch_waits_for_the_same_recorded_pass(tmp_path, monkeypatch):
    import rcp.runs.tasks.work_turn_runtime as runtime
    from rcp.agents import AgentEvent
    from rcp.runs.shared import _sse
    from rcp.transport import StateUnavailable
    ...
    service, request, execution = _one_result_app(tmp_path)
    resolved = work_module._resolve_work_execution(service, request, execution)
    turn, _staged = await work_module._stage_work_turn(
        service, resolved, tmp_path / "data", execution
    )
    pid_file = "/retained/turn.pid"
    execution.store.begin_remote_provider_pass(
        execution.operation_id, "execution-host", "/retained", pid_file, supervised=True
    )

    async def failed(*_args, outcome, **_kwargs):
        outcome.failed = True
        outcome.failure_kind = "delegation_unfinished"
        outcome.deferred_remote_pid_file = pid_file
        yield _sse(
            AgentEvent(
                event="error", text="Delegation expired.", failure_kind="delegation_unfinished"
            )
        )

    def unavailable(*_args):
        raise StateUnavailable("host disconnected before patch retention")

    monkeypatch.setattr(runtime, "_stream_agent_events", failed)
    monkeypatch.setattr(runtime, "_read_chat_patch", unavailable)
    frames = [
        frame
    ...
    assert turn.outcome.failure_kind == "delegation_unfinished"
    assert turn.outcome.remote_result_pending
    assert execution.store.unresolved_remote_provider_passes("execution-host", "/retained") == [
        (execution.operation_id, pid_file)
    ]
