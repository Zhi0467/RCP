...
def _claimed_delivery(tmp_path):
    ...
    worker = store.create_campaign_agent_task(
        AgentTaskRecord(
            ...
            authorized_by=authorizer,
            dispatch_authority=_campaign_authority("campaign", "worker"),
        ),
        role="worker",
    )
    messages = [
    ...
    for message in messages:
        store.record_campaign_message(message)
...
def test_claimed_inbound_mail_is_staged_as_one_exact_hearsay_only_batch(tmp_path) -> None:
    ...
    parsed = parse_campaign_mail_delivery((workspace / "messages.json").read_text(encoding="utf-8"))
    ...
    assert parsed.messages[0].authorized_by.display_name == "Campaign owner"
...
def test_legacy_v1_human_mail_without_sender_snapshot_still_parses(tmp_path) -> None:
    ...
    parsed = parse_campaign_mail_delivery(json.dumps(payload))
...
def test_campaign_mail_batch_validation_is_all_or_none(tmp_path) -> None:
    delivery = _claimed_delivery(tmp_path)
    payload = delivery.model_dump(mode="json")
...
def test_claim_prefix_stops_at_the_shared_count_and_exact_wire_boundary(
    ...
    two_message_delivery = campaign_mail_delivery(
        campaign_id="campaign",
        recipient_task_id="root",
        delivery_operation_id="mail-wake",
        messages=claimed,
    ...
    assert exact_boundary < CAMPAIGN_MAIL_MAX_BYTES
    monkeypatch.setattr("rcp.runs.campaign_mail.CAMPAIGN_MAIL_MAX_MESSAGES", 2)
    monkeypatch.setattr(
        "rcp.runs.campaign_mail.CAMPAIGN_MAIL_MAX_BYTES",
        exact_boundary,
    )

    selected = campaign_mail_claim_prefix(
        campaign_id="campaign",
        recipient_task_id="root",
        delivery_operation_id="mail-wake",
        delivered_at="2026-08-12T00:01:00+00:00",
    ...
    assert parse_campaign_mail_delivery(rendered) == two_message_delivery
    with pytest.raises(ValueError, match="exceeds .* bytes"):
        parse_campaign_mail_delivery(rendered + " ")
    ...
    smaller = campaign_mail_claim_prefix(
        campaign_id="campaign",
        recipient_task_id="root",
        delivery_operation_id="mail-wake",
        delivered_at="2026-08-12T00:01:00+00:00",
