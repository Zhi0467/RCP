...
from rcp.runs.campaign_delivery import (
    ...
    pending_campaign_mail_recipients,
...
def _start_campaign(
    ...
    *,
    ...
    return campaign, wait_for_task(tasks.store, root.operation_id, expect="succeeded")
...
def _arm_completed_graph_condition(
    ...
    *,
    watcher_id: str = "campaign-watcher",
) -> GraphWatcherRecord:
    ...
    blocker = Blocker(
        ...
        description="The campaign continues after this blocker resolves.",
        status="resolved",
    )
    watcher = arm_campaign_graph_condition(
        ...
        CampaignCommandContext(
            campaign=current,
            task=origin,
            request=CampaignRunRequest.model_validate(origin.request),
        ),
        WatchGraphArguments(
            condition=condition,
    ...
    assert watcher.origin_task_kind == "campaign"
...
def test_campaign_effect_ids_are_exact_and_graph_reconciliation_is_read_only(tmp_path) -> None:
    ...
    campaign, root = _start_campaign(tasks)
    ...
    assert store.campaign_message("planned-message") == message
...
def test_campaign_graph_watcher_wake_is_one_atomic_paid_actor_continuation(tmp_path) -> None:
    store = _store(tmp_path)
    stage = tmp_path / "campaign-stage"
    stage.mkdir()
    ...
    tasks = BackgroundAgentTasks(store, stream)
    campaign, root = _start_campaign(tasks)
    watcher = _arm_completed_graph_condition(store, campaign, root)
    before = store.campaign_budget_meter(campaign.campaign_id)

    wake_id = deliver_campaign_watcher_group(tasks, [watcher])
    ...
    assert store.campaign_invocation_role(wake.operation_id) == "orchestrator"
...
def test_busy_campaign_actor_leaves_completed_watcher_unclaimed_and_unspent(tmp_path) -> None:
    store = _store(tmp_path)
    stage = tmp_path / "campaign-stage"
    stage.mkdir()
    ...
    campaign, root = _start_campaign(tasks)
    watcher = _arm_completed_graph_condition(store, campaign, root)
    busy = tasks.start_campaign_turn(
    ...
    before = store.campaign_budget_meter(campaign.campaign_id)
    task_ids_before = [task.operation_id for task in store.campaign_tasks(campaign.campaign_id)]

    try:
        assert deliver_campaign_watcher_group(tasks, [watcher]) is None
        ...
        assert [
            task.operation_id for task in store.campaign_tasks(campaign.campaign_id)
        ] == task_ids_before
    finally:
        release_busy.set()
...
def test_pending_campaign_mail_coalesces_into_one_paid_message_wake(tmp_path) -> None:
    store = _store(tmp_path)
    stage = tmp_path / "campaign-stage"
    stage.mkdir()
    ...
    wake_id = deliver_pending_campaign_mail(
        tasks,
        campaign_id=campaign.campaign_id,
        recipient_task_id=root.operation_id,
    )
    ...
    wake = wait_for_task(store, wake_id, expect="succeeded")
    claimed = {
        message.message_id: message for message in store.campaign_messages(campaign.campaign_id)
    }
...
def test_reconciliation_retries_every_pending_canonical_actor(tmp_path) -> None:
    ...
    root_message = record_campaign_message(
        store,
        campaign_id=campaign.campaign_id,
        sender_role="human",
        sender_task_id=None,
    ...
    worker_message = record_campaign_message(
        store,
        campaign_id=campaign.campaign_id,
    ...
    assert pending_campaign_mail_recipients(
        store,
        campaign_id=campaign.campaign_id,
    ) == [(campaign.campaign_id, root.operation_id), (campaign.campaign_id, worker.operation_id)]
    wake_ids = reconcile_pending_campaign_mail(
        tasks,
        campaign_id=campaign.campaign_id,
    )
...
def test_bounded_mail_overflow_stays_pending_for_the_next_paid_retry(
    ...
    first_wake_id = deliver_pending_campaign_mail(
        tasks,
        campaign_id=campaign.campaign_id,
        recipient_task_id=root.operation_id,
    )
    ...
    first_batch = [
        message
        for message in store.campaign_messages(campaign.campaign_id)
    ...
    assert store.pending_campaign_messages(campaign.campaign_id, root.operation_id) == [messages[2]]

    retry_wake_id = deliver_pending_campaign_mail(
        tasks,
        campaign_id=campaign.campaign_id,
        recipient_task_id=root.operation_id,
    )
    ...
    retried = store.campaign_message(messages[2].message_id)
    assert retried is not None
    assert retried.delivery_operation_id == retry_wake_id
    assert store.pending_campaign_messages(campaign.campaign_id, root.operation_id) == []


def test_busy_campaign_actor_leaves_pending_mail_unclaimed_and_unspent(tmp_path) -> None:
    store = _store(tmp_path)
    stage = tmp_path / "campaign-stage"
    stage.mkdir()
    busy_entered = threading.Event()
    ...
    try:
        ...
        assert store.campaign_budget_meter(campaign.campaign_id) == before
        assert store.pending_campaign_messages(campaign.campaign_id, root.operation_id) == [message]
        assert store.campaign_messages(campaign.campaign_id) == [message]
    finally:
        release_busy.set()
        wait_for_task(store, busy.operation_id, expect="succeeded")


def test_not_yet_checkpointed_campaign_actor_leaves_mail_pending(tmp_path) -> None:
    store = _store(tmp_path)
