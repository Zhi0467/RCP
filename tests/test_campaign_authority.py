...
def _orchestrator_patch(*ops: dict, agent_action: str | None = None) -> Patch:
    return Patch(
        ...
        author="agent",
        producer="agent",
        summary="Advanced the campaign.",
        ops=list(ops),
        run_truth_scope=["repo-a"],
        authorized_by=fabricated_authorizer("Campaign researcher"),
        profile="orchestrator",
        task_id="campaign-turn-one",
...
def test_orchestrator_is_one_closed_profile_contract_and_campaign_scope() -> None:
    ...
    assert (
        require_apply(
            AgentTaskAuthority(
                operation_id="campaign-turn-one",
    ...
    with pytest.raises(ValueError, match="orchestrate requires an exact campaign"):
        require_dispatch(
            AgentDispatchAuthority(
                profile="orchestrator",
                task_contract="orchestrate",
...
def test_campaign_worker_has_campaign_scope_but_no_seat_or_control_scope() -> None:
    ...
    with pytest.raises(ValueError, match="campaign worker requires"):
        require_dispatch(
            worker.model_copy(
                update={
                    "scope": AgentDispatchScope(
