...
async function stagedDecisionPage(t, state) {
  ...
  const project = () => ({
    ...
    last_remote_sync_at: state.lastRemoteSyncAt,
    home_space_id: "space",
    ...
    skill_catalog: [],
    skill_defaults: {},
    experiment_control: state.experimentControl,
  ...
  const candidateDecision = (id, title) =>
    state.candidateAwaiting.includes(id)
      ? decision(id, title)
  ...
  const previewPayload = () => ({
    projection: {
      ...
      attention: attention(state.candidateAwaiting),
      primary_question: null,
      counts: { ...counts, decisions_awaiting_choice: state.candidateAwaiting.length },
      experiment_control: state.experimentControl,
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (path === "/api/health")
      ...
      if (state.refetchFails) {
        await route.fulfill({ status: 503, json: { detail: "Preview is unavailable." } });
        return;
      }
      if (state.previewResponses > 1 && state.slowRefetch)
...
function stagedDecisionState(overrides) {
  return {
    lastRemoteSyncAt: "2026-09-01T12:00:00Z",
    experimentControl: {},
    candidateAwaiting: ["dec/two"],
    previewResponses: 0,
    slowRefetch: false,
    refetchFails: false,
    ...overrides,
...
test(
  ...
  { timeout: 30000 },
  async (t) => {
    const state = stagedDecisionState({ slowRefetch: true });
    const { page, errors } = await stagedDecisionPage(t, state);
    ...
    state.lastRemoteSyncAt = "2026-09-01T12:01:00Z";
    state.experimentControl = { "exp/one": { health: "completed" } };
    state.candidateAwaiting = [];
...
test(
  "a staged preview that cannot refresh stops shadowing canonical state",
  { timeout: 30000 },
  async (t) => {
    const state = stagedDecisionState();
    const { page, errors } = await stagedDecisionPage(t, state);
    ...
    // The same re-poll, except its replacement preview is refused. Nothing asks
    // again, so a retained candidate would shadow the state this poll delivered
    // for as long as the edit stays staged.
    const reloaded = page.waitForResponse(
      (response) => new URL(response.url()).pathname === "/api/projects/demo",
    );
    const refetched = page.waitForResponse((response) =>
    ...
    assert.equal((await inboxOpen.textContent()).trim(), "2 open");
    // The refusal this test arranges is the only thing the console may carry.
    assert.deepEqual(
