...
test("job presentation preserves server facts, zero exit status and Cancel attribution", async () => {
  const { computeJobPresentation } = await import("../src/compute.ts");
  const job = {
    job_id: "job-1",
    label: "Training",
  ...
  assert.deepEqual(computeJobPresentation(job), {
    label: "Training",
    status: "opaque-status",
    backend: "slurm",
    exitStatus: "Exit status 0",
    cancellation: "Cancel requested by human-1 · 2026-09-06T10:00:00Z",
  });
  assert.deepEqual(
    computeJobPresentation({ ...job, label: null, exit_status: null, cancel_requested_by: null }),
    {
      label: "job-1",
      status: "opaque-status",
      backend: "slurm",
...
test("compute job status comparisons are rejected by TypeScript", async () => {
  const ts = await import("typescript");
  const filename = new URL("./compute-status-contract.ts", import.meta.url).pathname;
  const source =
    'import type { ComputeJobRecord } from "../src/types"; declare const job: ComputeJobRecord; job.status === "running";';
  const options = {
    noEmit: true,
    strict: true,
    skipLibCheck: true,
  ...
  const diagnostics = ts
    ...
    .filter((item) => item.file?.fileName === filename);
  assert.deepEqual(
