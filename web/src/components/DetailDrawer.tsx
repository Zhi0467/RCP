...
import { glossaryTermsForNode, humanFieldLabels, humanize, nodeTypeLabel, presentNode } from "../nodePresentation";
import type { BeliefTransition, Edge, GlossaryTerm, GraphNode, OntologyState, ValidationMessage } from "../types";
...
const ignored = new Set([
  "id", "type", "title", "standing", "created_rev", "updated_rev", "source_refs", "draft_touched", "origin",
  "extension_type", "extension_fields",
]);
...
export function DetailDrawer({
  ...
  const changes = useMemo(() => changedNodeFields(editBase, draft, ontology), [draft, editBase, ontology]);
  ...
  const presentedKeys = new Set([presentation.key, ...presentation.context.map((item) => item.key)]);
  ...
  const drawer = (
    <DraggableWindow className="node-detail-window" kind="detail">
      <aside className={`detail-drawer node-detail-drawer${node.draft_touched ? " draft-touched" : ""}`} role="dialog" aria-modal="false" aria-labelledby="drawer-title">
        <header data-drag-handle="true">
          <div>
            ...
            <div className="node-meta">
              ...
              {node.type === "evidence" && node.origin && <span className="node-origin">{originLabels[node.origin]}</span>}
            </div>
          </div>
          <button className="icon-button" aria-label="Close detail" onClick={close}><X size={18} /></button>
        </header>

        <div className={`drawer-content${editing ? " editing" : ""}`}>
          {editing ? (
            <form className="node-edit-form" onSubmit={(event) => { event.preventDefault(); stage(); }}>
              {editFields.map((field) => (
                <label className="node-edit-field" key={field.key}>
                  <span>{field.label}{field.kind === "list" ? " · one item per line" : ""}{field.nullable ? <span className="node-field-optional">Optional</span> : null}</span>
            ...
            {presentation.context.length > 0 && (
              <section className="node-context">
                ...
                <dl className="context-list">
                  {presentation.context.map((item) => <div key={item.key}><dt>{item.label}</dt><dd>{formatValue(item.value)}</dd></div>)}
                </dl>
              </section>
            )}
            ...
            {definitions.length > 0 && (
              <section className="node-glossary">
                ...
                <dl>
                  {definitions.map((term) => <div key={term.term}><dt>{term.term}</dt><dd>{term.plain_definition}</dd></div>)}
                </dl>
              </section>
            )}
              ...
              <section className="belief-history">
                ...
                <ol>
                  {transitions.map((transition) => {
                    ...
                    return (
                      <li key={`${transition.revision}-${transition.from_status}-${transition.to_status}`}>
                        <span className="belief-transition"><strong>{transition.from_status}</strong><span>→</span><strong>{transition.to_status}</strong></span>
                        <span className="mono">rev {transition.revision}</span>
                        {cause.nodeId ? (
                          <button type="button" onClick={() => onSelectNode(cause.nodeId!)}>{cause.label}</button>
            ...
            {Object.keys(node.extension_fields).length > 0 && (
              <section>
                ...
                <dl className="detail-list">
                  {Object.entries(node.extension_fields).map(([key, value]) => <div key={key}><dt>{humanize(key)}</dt><dd>{formatValue(value)}</dd></div>)}
                </dl>
              </section>
            )}
            ...
            <section>
              <div className="relations-control">
                <button className="relations-control-heading" onClick={onExploreRelations} aria-label={`Open DAG focused on relations for ${node.title}`}>
                  <Network size={15} /><strong>Relations</strong><small>{relations.length}</small>
                </button>
                {relations.map((edge) => {
                  const peerId = edge.source === node.id ? edge.target : edge.source;
                  const flags = edgeValidationFlags(edge.id, validationMessages);
                  return (
                    <div className={`relation-row${flags.length > 0 ? " has-flag" : ""}`} key={edge.id}>
                      <button type="button" onClick={onExploreRelations}>
                        <span>{edge.source === node.id ? edge.relation : `← ${edge.relation}`}</span>
                        <strong>{allNodes[peerId]?.title ?? peerId}</strong>
                      </button>
                      {flags.map((flag) => (
                        <span className="relation-flag" role="status" key={`${edge.id}-${flag.message}`}>
                          <AlertTriangle size={12} />{flag.message}
                        </span>
                      ))}
                    </div>
                  );
            ...
            {details.length > 0 && (
              <section>
                ...
                <dl className="detail-list">
                  {details.map(([key, value]) => <div key={key}><dt>{humanFieldLabels[key] ?? humanize(key)}</dt><dd>{formatValue(value)}</dd></div>)}
                </dl>
              </section>
            )}
            ...
            <section>
              ...
              {node.source_refs.length === 0 ? <p className="muted">No source excerpts attached.</p> : node.source_refs.map((source) => (
                <blockquote key={`${source.session_id}-${source.record_uuid}`}>
                  <p>{source.excerpt}</p>
                  <footer className="mono">{source.source} · {source.truth_repository} · {new Date(source.timestamp).toLocaleString()}</footer>
                </blockquote>
              ))}
            </section>
          </>}
        ...
        <footer className="drawer-actions">
          {editing ? <>
            <span className="node-edit-status">{changeCount > 0 ? `${changeCount} field${changeCount === 1 ? "" : "s"}` : "No changes"}</span>
            <div>
              <button className="button ghost" type="button" onClick={cancelEditing}>Cancel</button>
              <button className="button primary compact" type="button" disabled={mutationsDisabled || changeCount === 0} onClick={stage}>
                <Check size={14} /> Done
              </button>
            </div>
          </> : stagedNewNode ? <>
            <span className="node-edit-status">Staged node</span>
            <button className="button secondary compact" type="button" disabled={mutationsDisabled} onClick={onUnstage}><Trash2 size={14} /> Remove</button>
          </> : <>
            <button className="button ghost" onClick={onOpenChat}><MessageCircle size={15} /> Ask about this node</button>
            <div>
              <button className="button secondary" disabled={mutationsDisabled} onClick={beginEditing}>
              ...
              <button
                ...
                onClick={() => onStanding(node.standing === "contested" ? "asserted" : "contested")}
              >
                <X size={14} /> Disagree
              </button>
              <button
...
function formatValue(value: unknown): React.ReactNode {
  if (Array.isArray(value)) return <ul>{value.map((item, index) => <li key={index}>{formatValue(item)}</li>)}</ul>;
