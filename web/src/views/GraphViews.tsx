import { AlertCircle, ArrowUpRight, CircleDot, Eye, EyeOff, FlaskConical, Focus, Gauge, GitBranch, Maximize2, Minimize2, Orbit, Pin, PinOff, RotateCcw, Search, Workflow, X } from "lucide-react";
...
import { DAG_NODE_HEIGHT, DAG_NODE_WIDTH, useForceDag, type DagLayoutMode, type DagPosition } from "../hooks/useForceDag";
import { buildResearchPaths } from "../researchProjection";
import { buildRunTaskProjection, latestRunObservation, type AgentTaskGroup } from "../runProjection";
...
const scienceOrder: GraphNode["type"][] = ["research_question", "hypothesis", "decision", "experiment", "evidence", "blocker"];
...
export function ScientificView({ graph, trustView, onSelectNode, mutationsDisabled = false, onStageCustomNode }: ScientificProps) {
  ...
  return (
    <section className="view-panel research-view">
      <ViewHeading
        ...
        aside={hidden > 0 ? `${hidden} hidden` : `${projection.paths.length} question${projection.paths.length === 1 ? "" : "s"}`}
        action={<NewCustomNode
        ...
        <>
          <div className="research-path-list">
            {projection.paths.map((path) => (
              <article className="research-path" key={path.question.id}>
                ...
                <ResearchStage label="Ideas & decisions" nodes={path.ideas} onSelectNode={onSelectNode} />
                <ResearchStage label="Experiments" nodes={path.experiments} onSelectNode={onSelectNode} />
          ...
          {projection.unconnected.length > 0 && (
            <section className="research-unconnected">
              <header><strong>Not yet connected</strong><span>{projection.unconnected.length}</span></header>
              <div>
                {projection.unconnected.map((node) => <ResearchNodeCard node={node} onSelectNode={onSelectNode} compact key={node.id} />)}
              </div>
            </section>
          )}
        </>
...
export function DagView({ graph, trustView, onSelectNode, projectId, relationFocusNodeId, onClearRelationFocus }: DagProps) {
  const projection = useMemo(
    () => buildDagProjection(graph, trustView, relationFocusNodeId),
    [graph, relationFocusNodeId, trustView],
  );
  const naturalFocusNodeId = useMemo(() => dagFocusNode(projection.nodes, projection.edges), [projection]);
  const focusNodeId = relationFocusNodeId && graph.nodes[relationFocusNodeId] ? relationFocusNodeId : naturalFocusNodeId;
  ...
  const layout = useForceDag({ nodes: projection.nodes, edges: projection.edges, projectId, repulsion, mode: layoutMode });
  const typeCounts = useMemo(() => Object.fromEntries(dagTypes.map((type) => [type, projection.nodes.filter((node) => node.type === type).length])) as Record<GraphNode["type"], number>, [projection.nodes]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      const scroller = scrollRef.current;
      const focusNode = [...(canvasRef.current?.querySelectorAll<HTMLElement>(".dag-node") ?? [])]
  ...
  const repulsionLabel = repulsion < 650 ? "Gentle" : repulsion < 1050 ? "Balanced" : repulsion < 1500 ? "Strong" : "Wide";
  return (
    <section className="view-panel dag-panel">
      ...
      {relationFocusNodeId && graph.nodes[relationFocusNodeId] && (
        <div className="dag-relation-focus" role="status">
          <Focus size={15} />
          <span><strong>Relation focus:</strong> {graph.nodes[relationFocusNodeId].title}. This node and its directly connected neighbors stay bright; all other graph context is dimmed.</span>
          <button className="button compact secondary" onClick={onClearRelationFocus}><X size={13} /> Clear focus</button>
        </div>
      )}
        ...
        <div className="dag-shell" ref={shellRef}>
          <div className="dag-controls">
            <div className="dag-layout-controls">
              <span className="dag-control-label"><GitBranch size={14} /> Layout</span>
              <div className="dag-layout-switch" role="group" aria-label="DAG layout">
                <button className={layoutMode === "force" ? "is-active" : ""} type="button" aria-pressed={layoutMode === "force"} onClick={() => setLayoutMode("force")}>
                  <Orbit size={13} /> Force-directed
                </button>
                <button className={layoutMode === "flow" ? "is-active" : ""} type="button" aria-pressed={layoutMode === "flow"} onClick={() => setLayoutMode("flow")}>
                  <Workflow size={13} /> Research flow
                </button>
              </div>
              <span className="dag-control-label dag-projection-label">Projection</span>
              <div className="dag-layout-switch dag-projection-switch" role="group" aria-label="DAG ontology projection">
            ...
            <div className="dag-lenses" role="group" aria-label="Node brightness by type">
              <span className="dag-control-label"><Eye size={14} /> Brightness</span>
              {dagTypes.map((type) => {
              ...
              <button className="dag-tool-button" type="button" disabled={allBright} onClick={() => setBrightTypes(new Set(dagTypes))}>
                <Eye size={13} /> Brighten all
              </button>
              <button className="dag-tool-button" type="button" disabled={allDim} onClick={() => setBrightTypes(new Set())}>
                <EyeOff size={13} /> Dim all
              </button>
            </div>
            <div className="dag-physics-controls">
              {layoutMode === "force" && <label className="dag-force-control">
                <span><Gauge size={14} /> Repulsion</span>
                <input
              ...
              <button className="dag-tool-button" type="button" disabled={layout.pinCount === 0} onClick={layout.releasePins}>
                <PinOff size={13} /> Release all pins
              </button>
              <button className="dag-tool-button" type="button" onClick={() => {
          ...
          <div className="dag-zoom-plane" style={{ width: layout.width * zoom, height: layout.height * zoom }}>
          <div className="dag-canvas" ref={canvasRef} style={{ width: layout.width, height: layout.height, transform: `scale(${zoom})` }}>
            <svg width={layout.width} height={layout.height} aria-hidden="true">
              ...
              {projection.edges.map((edge, edgeIndex) => {
                ...
                const dimmed = projectionEmphasis === "dimmed"
                ...
                return (
                  <g className={`dag-edge-group ${dimmed ? "is-dim" : ""} ${projectionEmphasis === "neutral" ? "is-neutral" : ""}`} key={edge.id}>
                    <path className="dag-edge" d={geometry.path} markerEnd="url(#dag-arrow)" />
                    <text className="dag-edge-label" x={geometry.labelX} y={geometry.labelY}>{edge.relation.replaceAll("_", " ")}</text>
                  </g>
                );
              })}
            </svg>
            {projection.nodes.map((node) => {
              ...
              const projectionEmphasis = nodeProjectionEmphasis.get(node.id)
                ?? (ontologyProjection === "all" ? "emphasized" : "neutral");
              const dimmed = projectionEmphasis === "dimmed" || !brightTypes.has(node.type)
...
export function ExecutionView({ graph, trustView, onSelectNode, tasks, lastRefreshAt, onInspectTask }: ExecutionProps) {
  const nodes = projectNodes(Object.values(graph.nodes), trustView);
  ...
  const hasRuns = openBlockers.length > 0
      ...
      {!hasRuns ? (
        ...
        <div className="operating-sections">
          {(openBlockers.length > 0 || taskProjection.actionable.length > 0) && (
            <RunSection title="Needs action" count={openBlockers.length + taskProjection.actionable.length}>
              ...
              {openBlockers.map((node) => (
                <button className={`blocker-row${node.draft_touched ? " draft-touched" : ""}`} key={node.id} onClick={() => onSelectNode(node)}>
                  <AlertCircle size={16} />
                  <strong>{node.title}</strong>
          ...
          {experiments.length > 0 && (
            ...
            <article className={`run-card${node.draft_touched ? " draft-touched" : ""}`} key={node.id}>
              ...
              <p>{String(node.current_summary || node.objective || "No current summary yet.")}</p>
              {(node.attempts ?? []).map((attempt) => (
                <div className="attempt-row" key={attempt.id}>
                  <span className="attempt-seq">{String(attempt.sequence).padStart(2, "0")}</span>
                  <strong>{attempt.purpose}</strong>
                  <span className="attempt-outcome">{attempt.outcome || attempt.failure_reason || "No outcome recorded"}</span>
                  <span className="mono">{attempt.job_refs?.[0] ?? "—"}</span>
                  <span>{attempt.status}</span>
                </div>
              ))}
              {node.next_action && <div className="next-action"><ArrowUpRight size={14} /> Next: {String(node.next_action)}</div>}
            </article>
              ))}
            </RunSection>
          )}
    ...
    <section className="view-panel">
      ...
      {terms.length === 0 ? (
        ...
        <div className="glossary-table" role="table">
          <div className="glossary-head" role="row"><span>Term</span><span>Plain definition</span><span>Defined at</span></div>
          {terms.map((term) => (
...
export function AttentionOverview({ graph, onSelectNode }: Omit<Props, "trustView">) {
  ...
  const blockers = Object.values(graph.nodes).filter((node) => node.type === "blocker" && node.status === "open");
  return (
    <section className="view-panel">
      <ViewHeading title="Attention view" aside={`${proposals.length + ambiguities.length + blockers.length} open`} />
      <div className="attention-overview-grid">
        ...
        <OverviewCard label="Scientific blockers" value={blockers.filter((node) => ["scientific", "design"].includes(String(node.blocker_type))).length} />
      </div>
      <h3 className="section-label">Recommended next action</h3>
      {proposals[0] ? (
        <div className="recommended-action"><CircleDot size={17} /><strong>Understand and decide “{proposals[0].title}”</strong></div>
      ) : ambiguities[0] ? (
        <div className="recommended-action"><CircleDot size={17} /><strong>Resolve “{ambiguities[0].question}”</strong></div>
      ) : blockers[0] ? (
        <button className="recommended-action" onClick={() => onSelectNode(blockers[0])}><CircleDot size={17} /><strong>Inspect “{blockers[0].title}”</strong></button>
      ) : (
        <div className="quiet-empty compact"><CircleDot size={17} /><strong>No judgment queued</strong></div>
      )}
    </section>
  );
}

function ResearchStage({ label, nodes, onSelectNode }: {
  label: string;
  nodes: GraphNode[];
  onSelectNode: (node: GraphNode) => void;
}) {
  return (
    <div className="research-path-stage">
      ...
      <div className="research-stage-cards">
        {nodes.length > 0
          ? nodes.map((node) => <ResearchNodeCard node={node} onSelectNode={onSelectNode} key={node.id} />)
...
function ResearchNodeCard({ node, onSelectNode, compact = false }: {
  node: GraphNode;
  onSelectNode: (node: GraphNode) => void;
  compact?: boolean;
}) {
  return (
    <button className={`research-node-card ${node.standing} ${node.draft_touched ? "draft-touched" : ""} ${compact ? "compact" : ""}`} onClick={() => onSelectNode(node)}>
...
function AgentRunRow({ group, onInspectTask }: { group: AgentTaskGroup; onInspectTask: (operationId: string) => void }) {
  const latest = group.latest;
  return (
    <button className={`agent-run-row ${latest.status}`} onClick={() => onInspectTask(latest.operation_id)}>
      <span className="agent-run-state" />
      <span className="agent-run-copy">
        <span className="eyebrow">{agentTaskName(group.root)}{group.attempts.length > 1 ? ` · ${group.attempts.length} attempts` : ""}</span>
...
function dagFocusNode(nodes: GraphNode[], edges: Edge[]): string | undefined {
  ...
  return [...(roots.length > 0 ? roots : questions)]
    .sort((left, right) => (outgoing.get(right.id) ?? 0) - (outgoing.get(left.id) ?? 0) || left.id.localeCompare(right.id))[0]?.id
    ?? nodes[0]?.id;
}
...
function ViewHeading({ title, aside, action }: { title: string; aside: string; action?: React.ReactNode }) {
  return <header className="view-heading"><h2>{title}</h2><span className="view-aside">{aside}</span>{action}</header>;
}
...
function OverviewCard({ label, value }: { label: string; value: number }) {
  return <article className="overview-card"><span className="eyebrow">{label}</span><strong>{value}</strong></article>;
}
