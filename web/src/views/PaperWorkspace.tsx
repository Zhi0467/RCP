...
export function PaperWorkspace({
  ...
  const [config, setConfig] = useState<AgentRunConfig>(() => profileRunConfig(project.agent_profiles.paper_coach));
  ...
  const handledCoachTask = useRef<string | null>(
    tasks.find((task) => task.kind === "paper_coach" && task.status === "succeeded")?.operation_id ?? null,
  );
  ...
  useEffect(() => {
    const latestSessionId = tasks.find((task) => task.kind === "paper_coach" && task.native_session_id)?.native_session_id;
  ...
  const pendingCoachTask = pendingCoachTaskId
    ...
    : latestCoachTask && isActiveTask(latestCoachTask) ? latestCoachTask : null;
  useEffect(() => {
  ...
  const sessionTasks = activeSession
    ...
    : pendingCoachTask ? [pendingCoachTask] : [];
  ...
  const reviewStale = activeSession ? activeSession.graph_revision_examined < project.revision : false;
  ...
  if (paper.sync_state === "not_created") {
    return (
      <section className="paper-empty">
        <div className="paper-sheet-preview"><span># Introduction</span><i /><i /><i /></div>
        <div className="paper-empty-action">
          {saveError && <div className="paper-inline-error" role="alert">{saveError}</div>}
          <button className="button primary" disabled={creating} onClick={() => void create()}><FilePenLine size={15} /> {creating ? "Opening editor…" : "Create introduction"}</button>
        </div>
      </section>
    );
  }
        ...
        <header className="paper-toolbar">
          <div className="paper-status">
            ...
            <span className={`sync-state ${saveError ? "save-error" : paper.sync_state}`}>
              ...
              {saveError ? "save failed" : saving ? "saving" : dirty ? "unsaved changes" : syncLabel}
            </span>
          </div>
        </header>
        {saveError && (
          <div className="paper-save-error" role="alert">
            ...
            <span><strong>Your text is still in this editor.</strong> {saveError}</span>
            <button className="button compact secondary" onClick={() => setSaveError(null)}>Retry save</button>
          </div>
        )}
        {paper.sync_state === "conflict" && (
          <div className="conflict-banner">
            <AlertTriangle size={17} />
            <span><strong>The canonical introduction changed elsewhere.</strong> Automatic sync stopped. Choose which version survives.</span>
            <button className="button secondary" onClick={() => void resolve("use_canonical")}><RotateCcw size={14} /> Use canonical</button>
            <button className="button danger" onClick={() => void resolve("overwrite_canonical")}>Overwrite canonical</button>
          </div>
        )}
        <textarea
          ref={textarea}
          ...
          spellCheck
          value={content}
          onChange={(event) => { setContent(event.target.value); setDirty(true); setSaveError(null); }}
        />
      </div>
      ...
      <div
        ...
        onKeyDown={(event) => {
          if (event.key === "ArrowLeft" || event.key === "ArrowRight") {
            ...
            setEditorShare((current) => constrainEditorShare(current + (event.key === "ArrowLeft" ? -2 : 2)));
          }
          if (event.key === "Home" || event.key === "End") {
            event.preventDefault();
            setEditorShare(constrainEditorShare(
              event.key === "Home" ? MIN_EDITOR_SHARE : MAX_EDITOR_SHARE,
            ));
          }
        }}
        onPointerDown={(event) => {
      ...
      <aside className="coach-column">
        <header className="coach-header">
          ...
          <button className="button ghost compact" onClick={() => {
            ...
            setSubmitError(null);
          }}><MessageSquarePlus size={14} /> New chat</button>
        </header>
        <details className="session-history">
          ...
          <div className="session-strip">
            {sessions.length === 0 ? <span className="muted">No prior writing sessions</span> : sessions.map((session) => (
              <button className={activeSession?.native_session_id === session.native_session_id ? "active" : ""} key={session.native_session_id} onClick={() => {
                attachSession(session);
              }}>
                <span>{session.title || "Untitled coach session"}</span>
                <span className="session-meta">{session.provider} · rev {session.graph_revision_examined}</span>
        ...
        <AgentConfigControls
          ...
          defaultCollapsed
        />
        {reviewStale && <div className="stale-review"><AlertTriangle size={14} /> Project understanding changed since this session reviewed the draft.</div>}
        {!readiness?.authenticated && <div className="provider-unavailable"><WifiOff size={14} /> {readiness?.reason || `${config.provider} is unavailable.`}</div>}

        <div className="coach-transcript" aria-live="polite">
          {transcript.map((line, index) => <div className={`chat-line ${line.role === "agent" ? "coach" : line.role}`} key={`${line.taskId}-${index}`}>{line.role === "human" ? `You: ${line.text}` : line.text}</div>)}
          {submitError && <div className="chat-line error">{submitError}</div>}
          {coachActive && <div className="thinking"><i /><i /><i /> {activeTask.status_message}</div>}
          {activeTask && !coachActive && <div className="chat-task-blocked">Another agent task is active. You can keep editing and start coaching when it finishes.</div>}
        </div>
        <div className="coach-composer">
          <textarea aria-label="Message" value={message} onChange={(event) => { setMessage(event.target.value); setSubmitError(null); }} onKeyDown={(event) => {
            if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); void send(); }
          }} />
          <button className="icon-button primary" aria-label={activeSession ? "Resume session in background" : "Start coach task in background"} disabled={!message.trim() || Boolean(activeTask) || coachSubmitting || !readiness?.authenticated} onClick={() => void send()}><Send size={16} /></button>
        </div>
      </aside>
    </section>
  );
