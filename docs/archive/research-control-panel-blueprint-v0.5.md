...
**Status:** design locked for v1; §1.0, §5.5–§5.7 are stated intent, not v1 scope
**Date:** 2026-07-29
**Supersedes:** `research-control-panel-blueprint-v0.4.md` (2026-07-29)
...
+ ontology review and grilling 2026-07-29

---

## 0. What changed in v0.5

**The 2026-07-29 ontology amendment gives the project a north star, fixes what
the epistemic layer of the graph actually is, and makes the ontology itself
capable of evolving without invalidating history.**

Three concerns drove it. The ontology carried fields nobody could fill; belief
change was recorded as a state without a cause; and the schema was pinned in
place by the patch log, so an ontology meant to evolve could in practice only
grow.

**North star (§1.0, new).** RCP's three stages — personal research operating
system, multiplayer research environment, federated research network — are
written down as the thing v1 must stay consistent with. They are not v1 scope.
Their only claim on v1 is on decisions that are expensive to reverse, which in
practice means the shape of the ontology and the meaning of the patch log.

**Node model (§5.1, amended).**

- `confidence` is **removed** from `BaseNode`. It was never defined to the
  agent, never read by the backend, and rendered as a chip on every node. It
  duplicated `Hypothesis.status` and `Evidence.strength` while adding nothing.
- `Hypothesis.scope` is **added** and is **human-authored**. It records the
  boundary conditions under which the statement is asserted. This is the
  difference between a hypothesis and something another lab can use, and it is
  the field agents most reliably invent when they cannot ground it — so
  authorship is enforced in the validator, not requested in the prompt.
- `Evidence.origin` is **added**: `internal_run | external_publication |
  external_instance | analytic | unknown`. Prior work previously had nowhere to
  live, and this is the field that lets a federated import land without
  inventing a node type.
- `decision_type` was proposed and **rejected**; the reasoning is recorded in
  §5.8 so it is not re-proposed.

**Relations (§5.4, rewritten).** The prose "typical shape" table becomes
`RELATION_SPEC`: for each relation, its allowed source types, allowed target
types, and its **layer**. Enforced at `flag` initially, promoted to `reject`
once real graphs have shown which shapes are legitimate.

**Layers (§5.5, new).** The epistemic and action graphs were always latent in
the relation vocabulary; they are now declared. Two overlay projections over one
graph, joined at `Experiment`. No new node types.

**Belief causation (§6.4, new rule).** Any change to `Hypothesis.status` must
name a **cause** — an evidence edge, a `Decision`, a `Proposal` resolution, or a
human edit. No belief moves without a recorded reason, which makes the belief
trajectory readable from the log for free.

**Validator split (§6.4, amended — prerequisite for all of the above).** The
single validator contract of v0.4 becomes two rule sets. **Structural
invariants** run on admission *and* replay; **authoring rules** run on admission
only. Membership follows a derivation, not taste: a rule is structural iff its
verdict for a given patch can never change. This is what lets authoring rules
tighten over time without rewriting the past.

**Replay failure semantics (§6.4, corrected).** A patch that fails validation
during replay was skipped, and replay continued. That made the graph a function
of *(log, current code)* rather than of the log: a rule change silently deleted
content, and the drop cascaded into every later patch referencing it. Replay now
**halts** at the last good revision and marks the graph degraded.

**No replay cache (§6.4b, new).** Replay is one uncached pass over the whole log.
The checkpoint layer is removed: profiling showed replay was quadratic in graph
size because materialization deep-copied the entire graph per patch, and fixing
that left the cache saving under a second at a scale most projects never reach —
in exchange for a second source of truth about what the log means.

**Schema evolution (§5.6, intent).** The patch log is authoritative over the
current code, for schema as well as data. An ontology change is an **op in the
log**, so each patch is validated against the ontology in force at *its own*
revision, and a team's earlier work is never invalidated by their later ontology
edits. The extension model that follows from this — a base ontology that
extensions must declare a mapping *into*, not merely coexist with — is specified
as intent so v1 stays consistent with it.

**The transferable unit (§5.7, intent).** What leaves an instance is neither a
node nor the graph, but an RQ-scoped bundle. `SourceRef` is structurally
non-exportable, which makes provenance substitution — not filtering — the
boundary operation.

---

## 0.1 What changed in v0.4
...
- Refresh receives a bounded recent-change routing delta, patch replay is a
  single uncached pass over the whole log (v0.5, §6.4b), and operational
  task/session storage is retained under explicit limits.
...
- It writes at most one graph patch, exactly at `patch.json`. **That file is the
  only graph-change channel.** Chat may additionally write optional temporary
  preview files under the exact per-turn artifact directory RCP names in its
  prompt. Nothing patch-shaped is parsed out of stdout or the final assistant
  message.
...
### 1.0 North star — the three stages

RCP is a local-first, open-source system for managing research conducted by
humans and agents together. It represents research as a structured graph of
questions, hypotheses, decisions, experiments, evidence, blockers, and
artifacts, so that a project keeps a durable record of the *process* rather than
only its outputs.

The intended arc has three stages.

**Stage 1 — Personal research operating system.** One researcher runs RCP as the
control panel for their own work: projects, agent runs, experiments, the paper
surface, and a long-lived research history. **This is v1, and this document
specifies it.**

**Stage 2 — Multiplayer research environment.** A group shares one research
graph. Several researchers and their agents contribute asynchronously, delegate,
review each other's work, and accumulate institutional memory that outlives any
individual's context.

**Stage 3 — Federated research network.** Independent labs run their own
instances that interoperate through standardized research objects. A lab
selectively publishes structured artifacts, discovers related work, requests
replication, and lets agents reason over distributed research knowledge — while
its private research history stays private.

The goal is not to replace papers. It is to add a layer of structured research
communication underneath them, carrying the part of the process that
publication discards: dead ends, abandoned hypotheses, failed experiments, and
the reasons decisions were made.

**What the ontology is actually modeling.** Not artifacts, and not workflow. The
transformation:

```
uncertainty → action → observation → belief update
```

A pipeline reading of the node types — question, then hypothesis, then
experiment, then evidence — is the wrong reading and produces a prettier project
manager. Research is a belief-update process whose actions are chosen under
uncertainty; §5.5 makes that structure explicit.

**What this section may and may not do to v1.** Stages 2 and 3 are **not v1
scope** and confer no license to build for them (§1 Non-goals still holds).
Their only claim on v1 is over decisions that are expensive to reverse — which,
in practice, means two things and no others:

1. **The shape of the ontology**, because schema is baked into an append-only
   log and cannot be cheaply revisited.
2. **The meaning of the patch log**, because everything else is derived from it.

Every Stage 2/3-motivated decision in this document is additionally justified on
Stage 1 terms; where it is not, it was rejected (§5.8). The failure mode this
section exists to prevent is the opposite one: shipping a sophisticated ontology
nobody adopts, having optimized for a network that never formed.
...
**And it is not Stage 2 or Stage 3.** §1.0 records where this is going —
multiplayer, then federated — precisely so that the ontology and the patch log
can be chosen consistently with it. That is the whole of its authority over v1.
No sharing, syncing, publishing, discovery, import, or protocol work belongs in
v1, and §5.6–§5.7 are marked as intent for that reason. A reader who takes §1.0
as license to build federation has inverted its purpose: it exists to make the
*irreversible* decisions well, so the reversible ones can be deferred.
...
| Shipping form | local web app in a browser, **plus a desktop app (Tauri wrapper) — moved into v1 on 2026-07-31, see §12** | — *(nothing deferred)* |
...
### D1 — Source-derived, agent-native (clarified in v0.5)

The graph is derived by an agent, not by a deterministic collection layer. There
is no collection layer in v1.

**What "derived" reads is wider than conversations**, and v0.4's phrasing
("derived from agent conversations") understated it in a way that distorted the
v0.5 ontology review. The agent reads every authorized raw input in the run
scope: repository artifacts and documentation, experiment logs and job records,
collector dumps (D14), and conversations. Repository artifacts in fact rank
*above* conversation in the precedence the run prompt sets for current project
framing.

This matters to §5.1: how groundable a field is depends on this full surface,
not on transcripts alone. `Evidence.origin` is reliably observable across it;
`Hypothesis.scope` frequently is not, which is why the two fields have different
authorship rules rather than a single blanket policy.
...
fields only, never identity, standing, status, evidence references, relations,
attempts, or artifacts. `Hypothesis.scope` is in that allowlist and is
additionally *reserved* to the human (§5.1) — the only field an agent may not
author even when it could. It appends a human-authored patch through the
...
never cancels its task. All five task kinds share the same persisted lifecycle,
inspector, and progress, completion, failure, pause, resume, retry, and dismissal
language while the rest of the app remains usable. Placement follows the task's
surface: chat banners render only inside Chats, whose navigation item carries
active and unread-result state elsewhere; non-chat banners remain project-wide,
and Runs remains the complete project activity projection.

The receipt also persists a bounded event trail, native provider session id,
local or remote scratch-folder root, phase, attempt lineage, the immutable task
contract content, and a frozen duration estimate. Contract paths belong to one
scratch folder and are not durable: each new attempt re-stages its contract and
launches with a short pointer to that new execution-host path. Progress is
explicitly estimated: use the median of up to 20
...
receipt, never reassembles context, and tells the provider only to continue the
interrupted task. The saved prepared context must still exist and its graph
revision must still be current; otherwise Resume fails visibly and names Retry.
Reusing the same native session after a provider or patch failure is a distinct
correction continuation: it keeps the saved stage but adds the exact failure
diagnostic and retained patch pointer. Seed and Refresh launch requests never
accept a native session id:
...
Retry starts without the native session and receives a fresh scratch folder; it
never mutates or adopts an ancestor's writable workspace. For a cross-provider
Retry, RCP walks the complete attempt lineage and independently selects the
newest still-valid prepared corpus context and newest useful provider dispatch.
An intermediate attempt with no native session or progress cannot hide an older
useful transcript. The new attempt may read immutable staged inputs and a
provider-owned transcript from that lineage, but its contract and output path
are always re-staged into its own folder. The event trail reports context reuse
and provider-progress handoff separately, including why either fell back.
...
Every project has a first-class **Chats** projection. A compact left column lists
that project's node and project conversations; the selected transcript and
composer occupy the right. Project **Ask** opens or selects the project
conversation there. From a node detail, **Ask** opens a separate draggable chat
window without closing the independently draggable, modeless detail window.
Minimizing that chat returns it to Chats without cancelling work or losing typed
composer state. Chat task banners never render over another project projection;
the Chats navigation state is the notification while the human is elsewhere.
...
launched with its working directory on that folder. It writes at most one graph
patch, exactly at `patch.json`. A chat turn also receives one exact artifact
directory inside the reusable conversation scratch folder and may write
temporary preview files there. RCP reads the patch only when present, validates
it (§6.4), and appends it to the canonical state repo through the history
manager only when that turn carried human graph-change authority.

Three properties make this the whole design rather than an implementation
detail:

- **The graph deliverable is a file, never a message.** No patch is parsed out of
  the model's conversational output or final response. The labelled final
  assistant message remains the chat reply. Optional preview artifacts are
  discovered only as bounded, allowlisted direct files in the RCP-owned artifact
  directory; provider-specific directives, provider artifact roots, and URLs in
  the reply have no artifact meaning. HTML previews run in an opaque sandbox and
  cannot access or navigate RCP, open popups, submit forms, start downloads, or
  use ordinary network resource APIs. Inline JavaScript remains allowed and may
  navigate only its own isolated child frame, which can still cause a navigation
  request; the contract is parent isolation, not literal zero network traffic.
...
**`confidence` is removed in v0.5.** It was declared on `BaseNode` and never
defined anywhere else: no agent prompt or schema mentioned it, no backend code
read it, no test referenced it. It defaulted to `"medium"` on every node ever
created and surfaced in the detail drawer as a `"medium confidence"` chip — a
universal field whose only effect was to tell the reader something untrue.

It was also structurally redundant. Epistemic support belongs to the node type
that has it: `Hypothesis.status` for a hypothesis, `Evidence.strength` and
`Evidence.validity` for evidence. A universal `confidence` alongside those gave
each node two overlapping epistemic axes with no defined relationship — "status
`supported`, confidence `low`" had no meaning, and neither did "strength
`confirmatory`, confidence `low`".

**One axis, one owner** is the rule this leaves behind. `standing` is human
authority over the node (D3). `status` and the type-specific quality fields are
the agent's epistemic assertion. Nothing is universal that is not universally
meaningful.

**Development reset exception.** RCP is still pre-release, and every stored
`confidence` value is the same unused default. v0.5 deletes the field from the
code schema outright and does not migrate pre-v0.5 development history; fixtures
and local development state may be reset or reseeded. Do not add a compatibility
shim or legacy-key drop list for it. §5.6 governs schema changes made after this
clean v0.5 baseline, when preserving recorded user work becomes the contract.
...
class Hypothesis(BaseNode):
    ...
    scope: str                   # human-authored; boundary conditions (v0.5)
...
class Evidence(BaseNode):
    ...
    origin: Literal["internal_run", "external_publication",
                    "external_instance", "analytic", "unknown"]   # v0.5
...
#### `Hypothesis.scope` — human-authored boundary conditions (new in v0.5)

`scope` records the conditions under which the statement is asserted: *"up to
10B parameters, on these datasets, with this optimizer."* A hypothesis without
it is a slogan; a hypothesis with it is something another researcher — or
another lab — can act on. This is the entire useful residue of the `Claim` node
that was considered and rejected (§5.8): what distinguishes a claim from a
hypothesis is not epistemic type but **quantification boundary**, and a boundary
is a field, not a node.

**Authorship is enforced, not requested.** `scope` joins the human-editable
field allowlist. The agent may fill it **only** when a source states the
boundary explicitly; otherwise it raises an `Ambiguity` — *"under what
conditions does this hypothesis actually hold?"* — and leaves the field empty.

The reason is asymmetric cost. An empty `scope` is honest, visible, and
answerable by the one party who knows. An inferred `scope` is a fabricated
scientific claim, placed in precisely the field most likely to be exported and
believed by someone with no access to the reasoning behind it. There is no
`unknown` sentinel, because a sentinel would recreate the `confidence` failure:
a value that is always present and never means anything.

This is an instance of a general rule introduced in v0.5: **name the fields a
human must author, and enforce that in the validator rather than the prompt.**
A prompt is guidance; a validator is a contract.

#### `Evidence.origin` — where the observation came from (new in v0.5)

| value | meaning |
|---|---|
| `internal_run` | produced by this project's own execution |
| `external_publication` | reported in someone else's paper, talk, or preprint |
| `external_instance` | imported from another RCP instance as a structured object |
| `analytic` | derived by argument or calculation, not observation |
| `unknown` | provenance genuinely undetermined |

Before v0.5, prior work had nowhere to live: a result read in someone else's
paper became either an `Evidence` node indistinguishable from your own run, or a
`SourceRef` excerpt with no standing. Both are wrong, and the first is
dangerous — *"we measured 82"* and *"Chen et al. report 82"* carry different
weight in every downstream judgment a reader makes.

Unlike `scope`, this is **agent-filled and required**. It is reliably
observable: whether the researcher ran something or cited someone is plain in
the sources the agent already reads. `unknown` is permitted here precisely
because it is *rare and diagnostic* — a genuinely undetermined provenance is a
real finding worth surfacing, not a default.

`external_instance` is also the mechanism by which a Stage 3 import lands
without inventing an import node type (§5.7).

**Required is an authoring rule, not a model constraint** — the distinction
matters and generalizes to every future required field. The *model* carries a
default so that payloads written before the field existed still parse on replay
(§6.4, structural set). The *authoring rule* demands a real value on admission,
so no new evidence enters without one (§6.4, authoring set). Encoding
"required" in the model instead would make the field retroactive and drop
history.
...
> **Code/blueprint discrepancy, recorded not resolved.** The implementation's
> `Edge.relation` literal carries a fifteenth value, `has_subquestion`, which
> v0.4's enum above omits. `RELATION_SPEC` below includes it, since a
> question-decomposition edge is clearly intended and the Research view depends
> on it. The enum listing above is the one that is wrong.

#### `RELATION_SPEC` — typing and layers (new in v0.5)

v0.4 gave the shape of the graph as prose ("typical shape"), and nothing
enforced it. Endpoint existence was checked; **type compatibility was not**. A
`supports` edge from a `Blocker` to a `Decision` was legal, and the graph had no
way to notice.

That is tolerable only while nothing reads the structure. It stops being
tolerable in v0.5, because §5.5 introduces projections that filter by relation —
and a projection over an unconstrained edge soup renders nonsense with total
confidence. `RELATION_SPEC` replaces the prose table: for each relation, its
allowed source types, its allowed target types, and its **layer**.

| relation | source types | target types | layer |
|---|---|---|---|
| `has_subquestion` | `ResearchQuestion` | `ResearchQuestion` | epistemic |
| `has_hypothesis` | `ResearchQuestion` | `Hypothesis` | epistemic |
| `supports` | `Evidence` | `Hypothesis` | epistemic |
| `weakens` | `Evidence` | `Hypothesis` | epistemic |
| `refutes` | `Evidence` | `Hypothesis` | epistemic |
| `inconclusive` | `Evidence` | `Hypothesis` | epistemic |
| `contradicts` | `Evidence`, `Hypothesis` | `Hypothesis` | epistemic |
| `tests` | `Experiment` | `Hypothesis` | **seam** |
| `produces` | `Experiment` | `Evidence` | **seam** |
| `has_decision` | `ResearchQuestion` | `Decision` | action |
| `governed_by` | `Experiment` | `Decision` | action |
| `blocked_by` | `Experiment`, `Decision`, `ResearchQuestion` | `Blocker` | action |
| `requires_decision` | `Blocker` | `Decision` | action |
| `supersedes` | any | same type as source | meta |
| `duplicate_of` | any | same type as source | meta |

**Enforced at `flag`, not `reject` — deliberately, and provisionally.** This
table is derived from a prose list plus a handful of real edges. §5.4's original
shape was explicitly "typical," not exhaustive, and `relation` is a closed enum,
so an over-tight table leaves an agent no legal way to record real structure: it
must drop the edge or mislabel it. Both corrupt the graph more than a
questionable edge does.

So v0.5 ships the table as a flag, observes what real projects produce, and
promotes it to `reject` once the legitimate shape space is known. Under the
authoring/structural split (§6.4) that promotion is a one-line change with no
retroactive effect, so starting loose costs nothing.

**The leading candidates for widening**, named in advance so they are recognized
rather than rediscovered: `Evidence --supports--> Decision` (an observation
justifying a methodological choice) and `Blocker` targets beyond the three
listed. If either shows up repeatedly and legitimately, widen the table — do not
teach agents to work around it.

**This is contrasted with the causation rule (§6.4), which ships at `reject`.**
The difference is not strictness preference but epistemic status: the causation
rule's legal referents are knowable *a priori* and complete, so strictness costs
nothing. The legal *shape* space is empirical and is not yet known. Enforce what
you know; observe what you don't.

### 5.5 Layers and projections (new in v0.5)

The node types describe a pipeline only if you read them in listing order. What
they actually describe is two intertwined graphs that share a join.

**The epistemic graph** answers *what do we believe about the world?*

```
ResearchQuestion → Hypothesis → Evidence → belief update
```

**The action graph** answers *what did we do to learn?*

```
ResearchQuestion → Decision → Experiment → Attempt → Artifact
```

These are not two data structures. They are two **overlay projections over one
graph**, and the partition was already latent in the relation vocabulary — §5.4
merely names it. There are **no new node types**, and none are needed.

**The seam is `Experiment`.** Exactly two relations cross between the layers:
`tests` (Experiment → Hypothesis) and `produces` (Experiment → Evidence). That
is the correct answer to the recurring suspicion that `Experiment` is
overloaded. It is not overloaded — it is the **join** between the two graphs,
and a join is expected to have edges into both. An `Experiment` is not
knowledge; it is the action that produces knowledge.

Reading the graph this way corrects three habitual mistakes:

- A `Hypothesis` is not a child of a `ResearchQuestion` in the same sense a
  `Decision` is. One is what we might believe; the other is what we committed to
  do under uncertainty. They sit in different layers and were only ever siblings
  by accident of listing.
- A `Decision` does not answer a question. It determines the trajectory of
  future information gathering.
- Evidence is where the two graphs meet, which is why `Evidence.interpretation`
  carries so much weight: it is the point at which an action's output becomes a
  belief's input.

**`ExperimentAttempt` stays nested** (§5.2), and the layer model is the reason.
The experiment is the *scientific* object; the attempt is the *engineering*
object. A failed Slurm submission, a debugger run, a repaired smoke test, and
the final run are one coherent lineage inside one experiment. Promoting attempts
to nodes would put engineering events into a graph that exists for human
comprehension, which is the precise mechanism by which this becomes an
experiment tracker.

#### Layers are for reading, not for defining the export boundary

This distinction is load-bearing, and getting it wrong was the reason
`decision_type` was proposed and then rejected (§5.8).

Some decisions are *methodological* — "compare against reset baselines, not
random init" — and they condition what the evidence means. Under §5.4 every
`Decision` lands in the action layer, so an export defined as "ship the
epistemic layer" would silently drop exactly the context a replicator needs.

The fix is not to tag decisions. It is to stop treating a layer as a boundary:

> **Layers are a lens for reading one graph. The transferable unit is defined by
> reachability from a question, not by layer membership (§5.7).**

#### Relationship to the DAG view

Layer is **orthogonal to stage**, and does not replace it. The DAG's Research
flow columns follow semantic stage (question → hypothesis/decision →
experiment/blocker → evidence) and stay as they are. Layer is a second,
independent channel — a filter, a banding, or an emphasis — so the same canvas
can be read as "what do we believe" or "what did we do" without relayout.

---

### 5.6 Schema evolution and the extension model (intent, not v1 scope)

Everything in §5.4–§5.5 assumes the ontology will keep changing: authoring rules
tighten, fields sharpen, `RELATION_SPEC` widens. This section states the
conditions under which that is possible at all. **The mechanism described here
is not built in v1.** Its claim on v1 is narrow and stated at the end.

#### The governing decision

> **The patch log is authoritative over the current code — for schema as well as
> for data.**

v0.4 left this implicit and the implementation answered it the other way.
Because stored node payloads are revalidated on replay against a model with
`extra="forbid"`, the schema used for *reading* has to be a superset of every
schema ever used for *writing*. Fields could be added; they could never be
removed or renamed. The ontology was append-only in perpetuity, which is a poor
property for one that is meant to be iterated on, and an impossible one for
per-team extension.

Two consequences follow.

**Replay reads each patch under the ontology in force at its own revision**, not
under today's. A patch is a record of what was valid when it was written.

**A schema change is an op in the log.** Adding a field, removing one,
introducing a node type, deprecating a relation — each is appended like any
other change. Replay then reconstructs the *ontology* at revision N exactly as
it reconstructs the *graph* at revision N, and validates patch N against it.

This dissolves the migration problem rather than managing it. There is no schema
version to maintain, no compatibility matrix, and no drop-list of legacy keys in
code. A code-side legacy-key list was considered and **rejected** (§5.8): it is a
second source of truth about what the schema is, competing with the log, and it
goes stale exactly the way this document warns fallbacks do.

It also yields the guarantee a team needs before they will accept an ontology
editor: **their earlier work is never invalidated by their later ontology
edits**, because those edits are dated events in the same log, not retroactive
redefinitions.

#### What extension looks like

Once schema is data, a domain team can define its own ontology on top of the
base one — a bio group adding `Assay`, `Protocol`, `Measurement`; an ML group
adding `TrainingRun`, `Checkpoint`, `Benchmark` — through the UI, without
forking RCP.

**Strictness is preserved by structure, not by tolerance.** Base fields remain
fixed and typed; extension fields live in **one explicit namespace** on the
node, validated against the materialized ontology. Unknown *top-level* keys
still hard-fail. "Extensible" must not decay into "accepts anything."

**Every extension declares a mapping into the base ontology.** This is the whole
contract, and it is not optional:

| an extension type declares | example |
|---|---|
| which base type it specializes | `Assay` specializes `Experiment` |
| which layer it belongs to | action |
| which relations it may use | those legal for `Experiment` in §5.4 |

An extension may **specialize** the base. It may not **redefine** it.

The payoff is graceful degradation, and it is what makes Stage 3 possible at
all: an instance without the bio pack can read a bio project **at base
fidelity** — an `Experiment` carrying extension fields it does not interpret —
instead of failing to read it. This is the ordinary discipline of an extensible
wire format, applied to an ontology.

It also answers the question that motivates the whole design:

> The minimal universal ontology is **not the subset everyone agrees on**. It is
> the **projection target every extension must declare a mapping into.**

Which is why the base must stay small, and must stay about belief change:
`ResearchQuestion → Hypothesis → Evidence`, the two layers, the causation rule.
A base that grew to accommodate every domain would have nothing left to project
*onto*.

#### What is not extensible

The human-authority machinery is fixed and may not be extended, overridden, or
specialized:

- `standing` and its three states (D3), and who may set them (D2).
- The gated-approval path (D4) and the `Proposal` contract.
- The agent permission contract (`permissions_for()`), and the paper coach's
  absence of any write path.
- The structural invariants of §6.4.

A team that could define a node type whose `standing` an agent may set would
have opted out of the guarantee that makes their bundles worth anything to
anyone else. Extension is over *what research objects exist*, never over *who
holds authority*.

#### The only claim this section makes on v1

No dynamic schema engine is built in v1. Three decisions that v1 *does* make
must simply be chosen consistently with the above, at no additional cost:

1. The read-time rule set is **frozen and derived** (§6.4), never hand-curated,
   so a later ontology op cannot retroactively invalidate stored patches.
2. Replay **halts** on disagreement rather than filtering (§6.4), so schema
   evolution can never silently rewrite the past.
3. Schema evolution, when it arrives, extends the **op vocabulary** — so no v1
   mechanism may be introduced that would compete with it for authority over
   what the schema is.

---

### 5.7 The transferable unit (intent, not v1 scope)

The Stage 3 question — *what is the smallest research object that can leave one
instance and be useful inside another?* — has an answer that constrains v1's
ontology, and no other claim on v1.

It is neither a node nor the whole graph. It is an **RQ-scoped bundle**:

```
ResearchQuestion
  Hypothesis (with scope)          ← §5.1
    belief trajectory              ← derived from the causation rule, §6.4
    Evidence that moved it         ← with origin, §5.1
      artifact pointers
  Decisions reachable from the above (one hop into the action layer)
```

The internal action graph — attempts, blockers, operational decisions, the
conversations everything was derived from — stays private. This mirrors how a
lab already works: private branches, published releases. A paper is a lossy
projection of this bundle, and RCP's claim is that the bundle is the richer
object, not that it replaces the paper.

#### `SourceRef` is structurally non-exportable

Every node's provenance is a `SourceRef`: a machine name, a session id, a record
uuid, and an 800-character excerpt of a private conversation. None of that
means anything outside the instance that produced it, and some of it should
never leave.

This is a convenient accident: **the privacy seam is already structural rather
than a matter of policy.** But it means export is not filtering. It is
**provenance substitution** — replacing an internal `SourceRef` with a public
form (instance identity, bundle hash, revision) that supports verification
without disclosure.

Any v1 change that made `SourceRef` load-bearing for node *meaning*, rather than
for node *justification*, would break this and should be treated as a spec
conflict.

---

### 5.8 Rejected alternatives (new in v0.5)

Recorded so they are not re-proposed. Each was considered seriously during the
2026-07-29 ontology review.

**`Claim` as a node type — rejected.** The proposal was
`Evidence → Claim → Belief`, on the argument that research communities exchange
claims rather than hypotheses. But in this model the distinction collapses
correctly onto existing structure: `Evidence.interpretation` already performs
claim formation, and `Hypothesis.status` already records its acceptance. What a
`Claim` node would have carried that nothing else did is the **quantification
boundary**, and that is now `Hypothesis.scope` (§5.1). A node whose only unique
content is one field should be that field.

**`decision_type: methodological | operational` — rejected.** Proposed so an
epistemic-layer export would keep methodological context. Three reasons against.
It routes nothing, unlike `blocker_type` which reaches the human immediately —
and an enum that nothing acts on is how `confidence` began. The categories are
not exclusive: *"cap training at 100k steps, compute budget"* is simultaneously
a resource decision and a bound on every claim derived from it, and forcing a
choice there manufactures noise. And both of its jobs already have owners —
validity conditions belong to `Hypothesis.scope` and the existing
`Experiment.interpretation_rules`, while "what travels with an export" is
answered by reachability (§5.7), which is stronger than a self-declared tag.
Its rejection is what forced the clarification that layers are a lens and not a
boundary (§5.5).

**`ExperimentAttempt` promoted to a node — rejected**, restating v0.1. See §5.5:
the experiment is the scientific object, the attempt is the engineering object,
and the graph exists for human comprehension.

**`Observation` as a node distinct from `Evidence` — rejected.** It reimposes
the pipeline reading that §5.5 exists to dissolve, and splits one node across
the exact seam where the two graphs already meet.

**`Belief` / `ResearchState` as a node — rejected.** Belief state is not a
place in the graph; it is the *trajectory* of `Hypothesis.status` over
revisions, which the causation rule (§6.4) makes readable from the log without
any new object.

**A code-side `LEGACY_NODE_KEYS` drop-list — rejected.** Proposed to let removed
fields be stripped during op normalization. It is a second source of truth about
the schema, competing with the log, and stale by construction. Schema history
belongs in the log (§5.6).

**`Milestone` — rejected**, unchanged from v0.1 (§5.1).
...
class Patch:
    ...
    admission: Literal["accepted", "rejected"]
    admission_messages: list[ValidationMessage]
```

`admission` and `admission_messages` are RCP-owned receipt fields, not agent
output. They are omitted from the agent-facing schema and overwritten by the
history manager after admission validation. A rejected patch remains in the
append-only log with the exact verdict that kept it out of graph state; replay
can therefore skip that known rejection without re-running today's authoring
policy or mistaking it for log corruption. An accepted patch is the only kind
replay subjects to structural integrity validation.
...
3. **Direct prose edit:** one `update_nodes` operation containing one existing
   ...
   status, evidence references, relations, attempts, artifacts, or project
   settings.
...
recorded and shown. A rejected patch is still appended to the canonical log as
evidence, but none of its operations are materialized. Independently, the
operation record persists the raw patch text before validation so malformed
output remains recoverable too.

#### Two rule sets, not one (amended in v0.5)

v0.4 specified **one** validator contract shared by admission and deterministic
replay. That is withdrawn. It makes the materialized graph a function of *(log,
current code)* rather than of the log alone, so any tightening of a rule
retroactively changes history — which forecloses the ontology evolution that
§5.4–§5.6 depend on.

| | **Structural invariants** | **Authoring rules** |
|---|---|---|
| Runs on admission | yes | yes |
| Runs on replay | yes | **no** |
| A failure means | the log or the code is corrupt | the agent wrote something the policy of the day disallows |
| Response | halt (§6.4b) | reject the patch; recovery ladder applies (§6.4a) |
| May change over time | no | yes |

Membership is **derived, not curated**:

> A rule is structural **iff its verdict for a given patch can never change.**

Structural rules are properties of a patch relative to prior log state, and
prior log state is immutable — so their verdict is fixed forever. Id integrity,
`delete-forbidden`, revision monotonicity, and the approval/proposal semantics
qualify. Authoring rules encode policy in force when the patch was written;
their verdict changes when the policy does. The causation rule below, the
`RELATION_SPEC` table (§5.4), and required-field rules are authoring rules.

**Why replay validates at all.** If admission validation is complete and the log
is append-only and immutable, replay-time validation is *guaranteed to pass* —
it is the same pure function over the same inputs. It can only fail if the file
was altered outside RCP, or the code is wrong. So replay-time validation is not
a check on the patch; it is an **integrity check on the log and the code**,
which is worth keeping, and whose failure is therefore never routine (§6.4b).

#### The causation rule (new in v0.5)

> **No belief moves without a recorded reason.** Any operation that changes
> `Hypothesis.status` must name a `cause`.

A `cause` references exactly one of four kinds of referent:

| referent | when it applies |
|---|---|
| an **evidence edge** (`supports`/`weakens`/`refutes`/`inconclusive`) | an observation moved the belief |
| a **`Decision`** | a methodological commitment invalidated or settled it |
| a **`Proposal` resolution** | the human contested or approved the change (D3–D4) |
| a **human edit** | the human directly authored it |

Same-patch references are legal and resolved after the patch's own creates — the
common case is one patch creating the evidence, the edge, and the status change
together.

There is deliberately **no `unknown` escape hatch.** With four referent kinds
covering observation, method, human authority, and direct authorship, an agent
that can name none of them is almost certainly asserting a belief change it
cannot justify. A sentinel would reproduce the `confidence` failure exactly: a
value always present and never meaningful.

Enforced at **`reject`**, as an authoring rule. That is affordable because §6.4a
hands validation errors back to the same live session for two rounds, so a
missing `cause` is a correctable nudge rather than a destroyed run.

**What this buys.** The belief *trajectory* — the thing the ontology exists to
model (§1.0) — becomes derivable from the append-only log with no new node type
and no extra storage: *"`hyp/replay-preserves-plasticity` was `active` at rev 4,
`supported` at rev 9 because of `ev/…`, `weakened` at rev 14 because of `ev/…`."*
Before this rule, belief was a scalar on the current node and its history was
unrecoverable.
...
4. **The patch is well formed but the graph rejects it.** Preserve the exact
   rejection and hand its authoring errors back to the same live session under
   the same two-round bound. Replay halts and canonical-state failures remain
   terminal; an agent-authored rejection is not canonical history yet and is
   normally correctable without human authority.

Three supporting rules: the patch text is persisted **before** validation runs,
so a patch that fails is still recoverable evidence rather than a deleted
artifact; an agent patch is validated under the append lock and enters the log
only when accepted, while human Sync and batch semantics remain unchanged; and
the scratch folder of a failed run is **retained**, aged out on a retention
...
attempt ends in failure and offers an explicit linked **Retry**. A same-provider
Retry after a non-limit failure may reuse the RCP-owned native session and saved
stage, but it receives the failure diagnostic rather than a bare resume
instruction. A clean or cross-provider Retry starts a new native session in a
fresh scratch folder, carries a pointer-based handoff to prior progress, and
leaves the failed attempt and its scratch artifacts unchanged until retention
cleanup.

**Reject:** *(each entry is structural or authoring per the test above; the
structural ones also run on replay, the authoring ones do not)*
...
- **A change to `Hypothesis.status` without a valid `cause`** (authoring, new in
  v0.5) — the causation rule above
- **An agent-authored `Hypothesis.scope`** that no cited source states
  explicitly (authoring, new in v0.5). The agent's path when it cannot ground
  the boundary is to raise an `Ambiguity`, not to write prose (§5.1)
- **A node payload carrying an unknown top-level key** (structural). Extension
  fields, when they exist, live in their own namespace and are validated against
  the materialized ontology (§5.6) — never as loose top-level keys
...
- **An edge violating `RELATION_SPEC`** (authoring, new in v0.5) — a relation
  whose source or target node type is outside its declared table entry (§5.4).
  A flag rather than a reject *provisionally*, until real graphs show which
  shapes are legitimate; §5.4 states the promotion condition.

### 6.4b Replay semantics (new in v0.5)

v0.4 did not say what replay should do when a stored patch fails validation, and
the implementation answered: skip the patch and continue. That is withdrawn, for
three reasons.

**It filters history instead of checking it.** Under skip-and-continue the graph
is a function of *(log, current code)*. Change a rule and content disappears —
not with a failure, but silently, leaving only a message in a list. The
append-only guarantee becomes nominal: an append-only set of files, not an
append-only history.

**It cascades.** Patches reference each other's outputs. Drop the patch that
created a node, and every later patch drawing an edge to it fails
`unknown-edge-source` and is dropped too, and so on down the log. A single
filter decision has unbounded blast radius, and the surviving graph is an
arbitrary subgraph no one authored.

**It breaks reproducibility across versions.** Two builds reading an identical
log can materialize different graphs. For an ordinary application that is a bug;
for a system whose premise is durable research history — and whose Stage 3 plan
is exchanging bundles between instances — it dissolves the premise, because a
bundle's meaning would depend on the recipient's build.

**The rule:**

> Replay **halts at the last good revision**. The graph is marked degraded,
> canonical writes are blocked pending repair, and the failure is surfaced
> prominently rather than recorded as one message among many.

A stale but coherent graph is strictly better than a silently lopped one. This
mirrors the existing quarantine-and-repair path for uncertain remote commits
rather than introducing a new failure mode. After the rule-set split above, this
condition should be nearly unreachable: only structural invariants run on replay,
and their verdicts cannot change.

**Replay is a single uncached pass, and must stay one.** An earlier draft of this
section required the validation rule-set version to join the checkpoint cache key,
because a checkpoint computed under an older rule set would otherwise preserve its
outcome — including a skipped patch — even after the rule that caused it was
fixed. That requirement is withdrawn along with the cache it described.

Measurement replaced it. Replay had been quadratic in graph size: materialization
deep-copied the entire `GraphState` before applying each patch, which accounted
for **98% of replay time** and made an 800-patch log take roughly 15 seconds to
open. Copying only the containers and sharing their contents — safe because
`_apply_patch` never mutates a contained model in place — removed it. Full replay
is now ~280 ms at 2,000 patches and ~1.2 s at 5,000, of which the unavoidable
read-and-parse is a third.

At that cost a replay cache buys under a second at a scale most projects never
reach, while adding a second source of truth about what the log means and a
staleness surface of its own. So there is no checkpoint layer: **the graph is
always materialized by replaying the whole log.** If replay ever becomes a real
cost again, fix the per-patch work before reintroducing a cache over it — the
expensive thing was never where the machinery was.
...
- Successful ingest scratch folders and graph-run outputs are removed
  immediately. A chat's reusable conversation scratch folder remains temporary
  until the normal stage-retention sweep so its native session can resume and
  its previews can reopen while present. Failed, paused, and interrupted
...
Patch-log replay remains authoritative and append-only, and is **uncached**:
every load replays the entire log. v0.4 specified a derived checkpoint written
every 100 accepted revisions, with loading resuming from the newest
prefix-hash-verified snapshot; **v0.5 removes it** (§6.4b). Replay is fast enough
that the cache saved under a second at a scale most projects never reach, and a
second stored answer to "what does the log mean" is a liability that startup
time no longer justifies.
...
- Inferred claims carry source refs, and a belief change carries a `cause`
  (§6.4).
...
RCP shares Margin Dev's visual grammar rather than literally copying its
catalog. Use a restrained paper, paper-deep, sheet, walnut, and oxblood system;
semantic accents only where color communicates type or state; compatible
editorial, UI, and mono type stacks; warm rules and shadows; and tactile
bound-book materials. Do not rotate project cards through decorative colors,
introduce a parallel cold palette, retain isolated blue controls, or make every
surface a different theme. Project covers share one oxblood base and differ by
texture only. RCP keeps its own layout, information architecture, labels, and
behavior while sharing Margin's color, texture, and material language.
...
Settings also exposes one local **Display** preference for interface text size.
It is application-wide rather than project state, persists on the client, and is
never published to a manifest or `.research/`. In the Tauri window,
Command–Minus, Command–0, and Command–Plus control that same bounded preference;
the shell's built-in webview zoom shortcuts are disabled so one keypress applies
one change. The desktop applies it through native webview zoom rather than a CSS
root transform, preserving pointer and floating-window coordinates; the browser
does not expose the control. This presentation preference does not mutate the
DAG's own zoom.
...
The compact project header keeps only the back control, project name/status,
Sync, icon-only accessible Activity and Refresh controls, and **Ask**. It has no
product wordmark, product logo, or revision label. Horizontal navigation is
**Overview**, **Inbox**, **Research**, **Runs**, **Glossary**, **Paper**,
**Settings**, and **Chats**. Inbox carries a colored unresolved-attention count;
Chats carries active or unread chat state while its banner is hidden outside
that panel. DAG is a
subpanel switch inside Research rather than a primary destination. Glossary and
Paper use distinct icons. Only one panel is visible at a time. The trust-view
picker appears on graph projections instead of occupying every page.

Header controls are grouped by meaning: the labeled actions **Sync / Ask** sit
together; the self-explanatory icon utilities **Activity / Refresh** sit
together after them. A previously opened project renders from one durable local
display snapshot while its authoritative state refreshes in the background.
That snapshot is never an authority or agent input, and canonical mutation
controls remain unavailable until the refresh confirms current state. Remote
refresh work must not block unrelated API requests on the web event loop.
...
### Inbox view — the contained attention surface
...
### Research / DAG subpanel — a bounded interactive projection
...
### Draggable detail window — on click of any object

The detail is modeless: it has no page-blocking scrim, the project beneath stays
interactive, and its header drags the window while keeping it reachable within
the viewport. Opening node chat keeps the detail visible and opens a separately
draggable chat window at a non-overlapping initial position.
...
**Desktop app (Tauri wrapper) — moved into v1 on 2026-07-31.** It ships as a
second entrance to the same server, never a second implementation: both the
terminal and the application reach one FastAPI process, one data directory, and
one React frontend.

The claim this paragraph used to make — that browser-first meant the frontend
could "be wrapped later with little work" — turned out to be wrong, and the
correction is the reason the work is scoped the way it is. Wrapping the frontend
is indeed cheap. What is not cheap is that the server was written for exactly one
entrance: `rcp.lock` records a PID and no address, so a second entrance cannot
find the server it must not duplicate; `serve` always replaces the current owner,
which is right for a typed command and wrong for an application launch; the
backend invokes npm on every start and reads one of its own modules as a source
file on disk, neither of which survives packaging; and the application version
disagrees with itself in three places, so no compatibility rule can be written
against it.

Those are defects in the single-entrance server, not desktop features, and they
are fixed first. See [S28](acceptance/S28-one-backend-two-entrances.md),
[S29](acceptance/S29-refuse-instead-of-taking.md), and
[S34](acceptance/S34-packaged-app-needs-no-toolchain.md); the shell's own
promises are [S30](acceptance/S30-desktop-window-is-not-the-app.md),
[S31](acceptance/S31-quit-stops-what-it-started.md),
[S32](acceptance/S32-artifacts-in-the-desktop-window.md),
[S35](acceptance/S35-packaged-environment-parity.md), and
[S36](acceptance/S36-updating-never-interrupts-work.md).

Three decisions shape the shape of the work, all settled 2026-07-31:

- **Two bundles.** A thin dev shell that loads the checkout and needs no
  packaging, beside a self-contained release app — the pattern Margin already
  uses. Debugging the shell must not require running the packaging line.
- **The application updates itself** from a published manifest, on request and
  never automatically, deferring while agent work is in flight.
- **Identity and ownership are standing properties, not launch facts.** Both
  entrances re-verify which server they are talking to, and Quit re-derives
  whether it owns the backend before stopping anything.

First release targets Apple Silicon macOS.
...
| Scratch-folder isolation | Codex: spawn a run with one selected repo → selected raw content is readable in place but writes into it fail; other project repos are not raw-accessible; global graph and `research.md` remain readable; writes succeed only in scratch. An ingest run writes `patch.json`; chat may also write direct files under its exact per-turn artifact directory. Claude: verify the same requested behavior and prompt contract, while recording that `acceptEdits` + `--add-dir` does not provide an OS-enforced repository-write denial |
...
| Failure retains evidence | After a failed ingest run, the scratch folder still exists and the patch text that failed validation is retrievable; after a successful ingest run the scratch folder is gone. A successful chat retains its reusable conversation folder only until normal stage expiry |
...
| Replay determinism and cost | Replay a multi-thousand-patch log twice and against the committed `graph.json` → byte-identical every time, with no cached prefix involved. A patch that fails a structural invariant halts replay at the last good revision and marks the graph degraded; it is never skipped over (§6.4b) |
...
| Background lifecycle and retention | Start local and remote Seed/Refresh attempts, navigate freely, inspect phase/event/progress state, pause each without orphaning its provider process, and resume from an RCP-checkpointed native session plus its stable scratch folder. Caller-supplied and legacy untrusted session ids are refused. Restart RCP mid-run → normal shutdown persists `paused`; hard interruption persists `interrupted`; pause, interruption, and terminal failure retain scratch/output for 7 days, successful ingest removes it immediately, successful chat keeps only its temporary reusable conversation stage, and linked Retry starts a fresh folder. Apply the task 100/90-day union with active retry lineage and writing-session 50/180-day union; no failed attempt materializes a partial patch, and only success reaches 100% |
...
- ~~Desktop packaging~~ — moved into v1 on 2026-07-31; see §12
