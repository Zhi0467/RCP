...
> [S31](../../acceptance/S31-quit-stops-what-it-started.md) is `implemented` and
> last passed 2026-08-12 — an owned long-running Work task paused rather than
> interrupted, so the owned-sidecar, takeover, and forced-timeout paths that were
> outstanding here are covered. Retained only as historical execution context.
