...
1. Start a campaign and let the orchestrator take a turn that decides the
   governed Decision, sets an Experiment status, and resolves the Blocker.
...
5. Read the Experiment's readiness after the orchestrator decided its governing
   Decision.
...
- `readiness_satisfied_by_the_orchestrator_is_not_re_gated_behind_a_human_step`
...
worker on a Decision, and may still edit that Decision itself — a refusal in
step 4 must not be read as a restriction in step 1.
