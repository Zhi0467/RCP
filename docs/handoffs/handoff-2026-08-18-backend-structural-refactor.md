...
- **Status:** Implementation is in progress on `main`. The audit correction,
  ...
  watcher-maintenance task owners and the byte-identical Work, Discuss,
  paper-Coach, Seed/Refresh Graph, branch-merge, episode-report, and Auto-research
  stream package rehomes are committed and verified; the remaining detailed
  Phases 6–7 work remains the work order.
...
| 2026-08-19 | 5 / views    | Result-view list, GET/HEAD preview, Keep, and their three HTTP-only byte/visibility helpers move together. `ApiServices` carries the original `KeyedLocks` object, so Keep still serializes against task start, Resume, and Retry on the same lock instance. No second lock, optional fallback, or dynamic `app.state` mirror was added. | Committed as `305ec5e`; 30 main-agent focused tests, full backend suite, Ruff, and both hook passes are green. |
| 2026-08-19 | 5 / projection | `ProjectDisplayCache.transition_payload` now owns completion of a coherent transition projection: graph/control/guidance/head come from the candidate projection, while the explicitly requested live-operational path still reconciles Experiment runtime and Stop settlement before attaching the matching current episode. Sync preview keeps the non-reconciling candidate path. The enclosing `create_app` closure was removed rather than re-exported. | Committed as `f87a18e`; 52 main-agent focused tests, full backend suite, Ruff, and hooks are green. |
...
| 2026-08-19 | 5 / project state | All fifteen remaining project-scoped handlers and sixteen route entries move into one membership-gated `api/project_state.py` router. The route model moves with them; the existing catalog, store, identity, background-task, display-cache, project-service, and registered-project dependencies are sufficient, so `ApiServices` gains nothing. Snapshot generation/commit fencing, cache-only revision heartbeat, leave-time episode fencing, GET/HEAD repository preview, settings/provider validation, project-vs-global cache distinction, response models, and error mappings are unchanged. Exactly seventeen route-owned tests move from `test_api.py` into `test_api_project_state.py`; their function ASTs and the copied experiment fixture helper are unchanged, while cross-surface tests remain in the original module. | Committed as `8831e14`; the main-agent project-state/membership/repository/invitation/remaining-API set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
| 2026-08-19 | 5 / health | The final route body moves into `api/health.py`, leaving `api/app.py` composition-only. One frozen, slotted `HealthComposition` carries the exact immutable startup snapshot—server metadata, explicit agent mode, optional default project name, space id, and space kind—rather than inferring mode from launcher type or project identity from catalog order. The route still reads live space name and active-task count from the store and project count from the catalog; SQL statuses, version source, conditional `project` key, public-team middleware path, unknown-query behavior, and router order are unchanged. Two pure health tests move into `test_api_health.py` with identical ASTs; cross-surface health checks remain with their owning flows. | Committed as `e777417`; the main-agent health/runtime/auth/membership/remaining-API set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. Phase 5's zero-route-body target is proven by AST inspection. |
...
| 2026-08-19 | 6 / Discuss package | The Discuss executor moves byte-for-byte from `runs/discuss.py` to `runs/tasks/discuss.py`; the 644-line SHA-256 remains `a9052152760ae63297998ac211adf2019e66de8d7437eecbe19329a5ca560945`. All nine production, test, and monkeypatch references now name the task owner. The old path is absent, with no shim, package re-export, policy edit, or import cycle. | Committed as `c0f6f68`; the main-agent attachment/retention/prompt/retry/API set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
| 2026-08-19 | 6 / Coach package | The paper-Coach executor moves byte-for-byte from `runs/coach.py` to `runs/tasks/coach.py`; the 372-line SHA-256 remains `a28977ff7b31de203932f130a757235dfc171ed1d3b3ae112205b9d75734127b`. All four API-composition, task-route, retry, and test references now name the task owner. The old path is absent, with no shim, re-export, policy edit, or import cycle. | Committed as `1106f8a`; the main-agent Coach/API/retry/authority/task-policy/prompt set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
| 2026-08-19 | 6 / Graph package | The Seed/Refresh Graph executor moves byte-for-byte from `runs/graph.py` to `runs/tasks/graph.py`; the 1,316-line SHA-256 remains `6e6f2deeed2bc67754ba1f55a0859c3b4bf635f5184ba0f66e255efcd674fe4d`. All eight production, test, and monkeypatch references now name the task owner. The first full run correctly failed the current-document link check because `AGENTS.md` still linked the removed path; that canonical recovery link moved with the owner, the focused documentation regression passed, and the final full run is green. The old module path is absent, with no shim, re-export, policy edit, or import cycle. | Committed as `b1c52f3`; the main-agent ingestion/context/contract/graph-validator/patch-validator/API/background set, corrected full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
| 2026-08-19 | 6 / branch-merge package | The hidden branch-merge task executor moves byte-for-byte from `runs/branch_merge_task.py` to `runs/tasks/branch_merge.py`; the 391-line SHA-256 remains `8f517d15c308b6654fc79c6356520dce8a96062f67e49c404431785046817721`. Its three composition, direct-test, and current-document inventory references move with it. The shared semantic merge algorithm at `runs/branch_merge.py` is untouched, and the old task path is absent with no shim, re-export, policy edit, or import cycle. | Committed as `612aa76`; the main-agent merge semantic/task/API/history/target/documentation set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
| 2026-08-19 | 6 / episode-report package | The hidden episode-report task executor moves byte-for-byte from `runs/episode_report.py` to `runs/tasks/episode_report.py`; the 704-line SHA-256 remains `7de13d6249130e2a44fe477cb9dc184129e60283d2878668e2216afbb900c9d3`. All six API composition, background, wrap-up, direct-test, and task-policy references now name the task owner. Episode admission and wrap-up orchestration remain above the task executor, and the old path is absent with no shim, re-export, policy edit, or import cycle. | Committed as `ae32db2`; the main-agent report/prompt/API/wrap-up/lifecycle/background/task-policy/documentation set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
| 2026-08-19 | 6 / Auto-research stream package | The Auto-research orchestrator/worker task executor moves byte-for-byte from `runs/auto_research_stream.py` to `runs/tasks/auto_research_stream.py`; the 2,021-line SHA-256 remains `ecd65108c4661044afcd1f36e4a947994c36c1222581c636ed19d99faea474a9`. All six API-composition, direct-test, package-import, and monkeypatch references now name the task owner. Long-lived episode admission and lifecycle policy remain above this already-admitted executor, and the old path is absent with no shim, re-export, policy edit, or import cycle. | Committed as `e744da5`; the main-agent Auto-research stream/S77/API/background set, full backend suite, Ruff, exact-new-file hooks, and all-files hooks are green. |
...
- `_retry_deliverable_is_unchanged` was also called by the ordinary Work initial
  ...
  `runs.tasks.work` would have left a hidden compatibility dependency, while
  importing `runs.tasks.work` from the new maintenance owner would have created
  a cycle.

After the move, `tasks/work.py` imports only the maintenance processor from the new
owner. Work and Auto-research stream both import the retry comparison directly
from `runs.shared`; the new owner imports no symbol from `runs.tasks.work`.
...
App composition selects that entry point from the persisted
`auto_research_child_work_for_operation(execution.operation_id)` route before
ordinary Work dispatch. The route—not trigger wording—is the authority boundary;
when it exists, dispatch fails loudly through the child path rather than falling
back to an ordinary Work session.
