...
{_optional_pointer("starting instruction", instruction_path)}{_optional_pointer("delivered mail", messages_path)}Repositories and operational context:
...
- Available verbs are `validate`, `status`, `spawn`, `pause`, `resume`, `stop`, `message`,
  `watch-graph`, and `finish`. Read each JSON response and use returned worker ids in later calls.
- Normal campaign completion is explicit: after every admitted child turn has settled and its
  outcome is reflected, invoke `{command_client} finish --key <stable-key>`. Sleeping on a watcher
  or mail is not completion. A successful finish fences new work and schedules the concluding
  report in this same orchestrator session.
...
- graph: `{graph_path}`
- research rendering: `{research_path}`
{_optional_pointer("delivered mail", messages_path)}Repositories and operational context:
{_repositories(repositories)}
...
Current repository pointers replace every repository pointer in the original contract for this
continuation:
{_repositories(repositories)}

Staged command client for this turn:
- Command prefix: `{command_client}`
- The original verb, idempotency, unknown-call, worker-seating, and no-polling rules still apply.
- If the campaign's research is complete and every admitted child turn has settled, invoke
  `{command_client} finish --key <stable-key>` once. Waiting for mail or a watcher is not finish.
