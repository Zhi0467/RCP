...
class AppStoreBase:
    ...
    _STORAGE_SCHEMA_MIGRATIONS: ClassVar[tuple[tuple[int, str], ...]] = (
    ...
    def _initialize(
        ...
        *,
        ...
        _schema_template: bool = False,
        _schema_capture: list[tuple[str, str, str, str]] | None = None,
    ) -> str | None:
        with self.connection() as connection:
            try:
                ...
                connection.rollback()
                connection.execute("PRAGMA journal_mode = WAL")
            # Schema-normalizing migrations rebuild a small closed set of tables.
            # Keeping foreign-key rewriting off during the one transaction lets
            # those tables retain their canonical names; foreign_key_check below
            # remains the commit fence.
            ...
            return self._run_storage_schema_migrations(
                connection,
                requested_space_kind,
                initial_space_id=initial_space_id,
                initial_space_name=initial_space_name,
    ...
    def _run_storage_schema_migrations(
        ...
        requested_space_kind: SpaceKind | None,
        *,
        initial_space_id: str | None,
        initial_space_name: str | None,
        issue_bootstrap: bool,
        require_new: bool,
        ...
        def migrate_legacy_startup_schema(migration_connection: sqlite3.Connection) -> None:
            nonlocal bootstrap_code
            bootstrap_code = self._migrate_legacy_startup_schema(
                ...
                database_is_new=database_is_new,
            )
    ...
    def _migrate_legacy_startup_schema(
        ...
        *,
        ...
        """Own every retained pre-ledger shape and build the one current baseline."""
        ...
        if not identity_table_exists:
            legacy_database = not database_is_new
        ...
        connection.execute(
            "CREATE INDEX IF NOT EXISTS watchers_due ON watchers(status, next_check_at, created_at)"
        )
        connection.execute(
        ...
        self._ensure_legacy_auto_research_archives(connection)
        connection.execute(
    ...
    def _execute_sql_script(connection: sqlite3.Connection, script: str) -> None:
        ...
        for line in script.splitlines():
            ...
            if sqlite3.complete_statement(statement):
                ...
                statement = ""
        if statement.strip():
            raise RuntimeError("RCP storage schema script ended with an incomplete statement")

    @classmethod
    def _storage_schema_is_current(cls, connection: sqlite3.Connection) -> bool:
        ledger_exists = (
            connection.execute(
                "SELECT 1 FROM sqlite_master "
                "WHERE type = 'table' AND name = 'storage_schema_migrations'"
            ).fetchone()
            is not None
        )
        ...
        return tuple((int(row[0]), str(row[1])) for row in rows) == (cls._STORAGE_SCHEMA_MIGRATIONS)

    def _open_current_storage(
        self,
        connection: sqlite3.Connection,
        requested_space_kind: SpaceKind | None,
        *,
        ...
        """Validate identity and handle only the explicit team-bootstrap recovery write."""

        identity = connection.execute(
            """
            SELECT space_id, space_kind, space_name
            FROM space_identity WHERE singleton = 1
            """
        ).fetchone()
        if identity is None:
            raise RuntimeError("RCP space identity is unavailable.")
        ...
        recovering_team_initialization = bool(
            ...
            and issue_bootstrap
            and initial_space_name is not None
            and stored_space_kind == "team"
            and identity["space_name"] == initial_space_name
            and not users
        )
        if require_new and not recovering_team_initialization:
            raise ValueError("This RCP data directory already contains a space.")
        if requested_space_kind is not None and requested_space_kind != stored_space_kind:
            raise ValueError(
                f"RCP space is {stored_space_kind}; it cannot be opened as {requested_space_kind}."
            )
        if stored_space_kind == "personal":
            if len(users) != 1 or users[0].identity_kind != "local_owner":
                raise RuntimeError("A personal RCP space must contain exactly one local owner.")
            ...
            raise RuntimeError("A team RCP space cannot contain a local owner.")
        if not issue_bootstrap:
            return None
        if not recovering_team_initialization or initial_space_name is None:
            raise ValueError("A bootstrap code requires a new named team space.")
        connection.execute("DELETE FROM team_bootstrap_codes")
        bootstrap_code, code_id, code_hash = _new_enrollment_code("bootstrap")
        connection.execute(
            """
    ...
    @staticmethod
    def _storage_schema(connection: sqlite3.Connection) -> tuple[tuple[str, str, str, str], ...]:
        rows = connection.execute(
            """
    ...
    def _normalize_legacy_startup_schema(self, connection: sqlite3.Connection) -> None:
        ...
        for (object_type, name), row in expected.items():
            if object_type not in {"index", "trigger"} or actual.get((object_type, name)) == row:
                continue
    ...
    def _rebuild_storage_table(
        ...
        old_table = f"_storage_migration_old_{table}"
        if (
            connection.execute(
            ...
            raise RuntimeError(f"RCP storage migration scratch table already exists: {old_table}")
        old_columns = [str(row[1]) for row in connection.execute(f'PRAGMA table_info("{table}")')]
        connection.execute(f'ALTER TABLE "{table}" RENAME TO "{old_table}"')
        connection.execute(create_sql)
        new_columns = [str(row[1]) for row in connection.execute(f'PRAGMA table_info("{table}")')]
        retained = [column for column in new_columns if column in old_columns]
        if retained:
            columns = ", ".join(f'"{column}"' for column in retained)
            connection.execute(
                f'INSERT INTO "{table}" ({columns}) SELECT {columns} FROM "{old_table}"'
            )
        connection.execute(f'DROP TABLE "{old_table}"')

    def _validate_storage_schema(self, connection: sqlite3.Connection) -> None:
        expected = {(row[0], row[1]): row for row in self._baseline_storage_schema()}
        actual = {(row[0], row[1]): row for row in self._storage_schema(connection)}
        ...
        if missing or changed:
            ...
            if changed:
                details.append("changed " + ", ".join(f"{kind}:{name}" for kind, name in changed))
            raise RuntimeError("RCP storage schema validation failed: " + "; ".join(details))
        ...
        migrations = tuple((int(row[0]), str(row[1])) for row in migration_rows)
        if migrations != self._STORAGE_SCHEMA_MIGRATIONS:
            raise RuntimeError("RCP storage schema validation failed: migration ledger is invalid")
        foreign_keys = connection.execute("PRAGMA foreign_key_check").fetchall()
        if foreign_keys:
            raise RuntimeError("RCP storage schema validation failed: foreign keys are invalid")
        quick_check = [str(row[0]) for row in connection.execute("PRAGMA quick_check").fetchall()]
        if quick_check != ["ok"]:
            raise RuntimeError("RCP storage schema validation failed: SQLite quick check failed")

    @staticmethod
    def _ensure_legacy_auto_research_archives(connection: sqlite3.Connection) -> None:
        """Keep migrated Campaign rows in the baseline without creating live legacy tables."""

        statements = (
            """
            CREATE TABLE IF NOT EXISTS _legacy_campaign_invocations_archive (
                campaign_id TEXT, operation_id TEXT, role TEXT, created_at TEXT
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS _legacy_campaign_messages_archive (
                message_id TEXT, campaign_id TEXT, sender_role TEXT, sender_task_id TEXT,
                authorized_space_id TEXT, authorized_user_id TEXT,
                authorized_display_name TEXT, recipient_task_id TEXT, control_node_id TEXT,
                body TEXT, created_at TEXT, delivered_at TEXT, delivery_operation_id TEXT
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS _legacy_campaign_recoveries_archive (
                recovery_id TEXT, campaign_id TEXT, operation_id TEXT, purpose TEXT,
                failure_kind TEXT, retry_mode TEXT, attempts INT, max_attempts INT,
                status TEXT, next_attempt_at TEXT, diagnostic TEXT, admitted_operation_id TEXT,
                created_at TEXT, updated_at TEXT
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS _legacy_campaign_reports_archive (
                report_id TEXT, campaign_id TEXT, operation_id TEXT, ending TEXT,
                sha256 TEXT, html TEXT, created_at TEXT
            )
            """,
            """
            CREATE TABLE IF NOT EXISTS _legacy_campaigns_archive (
                campaign_id TEXT, project_id TEXT, root_operation_id TEXT, status TEXT,
                starting_instruction TEXT, invocation_ceiling INT, invocations_used INT,
                authorized_space_id TEXT, authorized_user_id TEXT,
                authorized_display_name TEXT, stop_requested_at TEXT, ending TEXT, error TEXT,
