...
"""
    ...
    def replace_conversation_inputs(self, sources: list[tuple[str, str]]) -> list[str]:
        ...
        files through the RCP machine: an empty-host conversation pointer already
        names a file on the execution host, and that host is the authority that
        can read it. The fixed ``inputs/conversations`` boundary also prevents a
        Claude ``--add-dir`` from exposing the source session's sibling files.

        Replacement fails closed. The provider is not launched unless every
        source was copied into a fresh directory and that directory replaced the
        previous turn's projection.
        """
        if self.root is None:
            raise RuntimeError("remote run stage is not open")
        target = self.root / "inputs" / "conversations"
        script = (
            "import json,os,shutil,sys,tempfile\n"
            "target=sys.argv[1]\n"
            ...
            "staged=tempfile.mkdtemp(prefix='.conversations-', dir=parent)\n"
            "try:\n"
            "    for source,relative in entries:\n"
            "        if not os.path.isfile(source):\n"
            "            raise FileNotFoundError(f'conversation input is unavailable: {source}')\n"
            "        if (os.path.isabs(relative) or relative in ('','.','..') or\n"
            "                os.path.normpath(relative).startswith('../')):\n"
            "            raise ValueError(f'conversation target is unsafe: {relative}')\n"
            "        output=os.path.join(staged, relative)\n"
        ...
        result = self._ssh(["python3", "-c", script, str(target), json.dumps(sources)])
        if result.returncode:
            raise StateUnavailable(
                result.stderr.strip() or "could not replace remote conversation inputs"
            )
        return [str(target / relative) for _source, relative in sources]

    def require_conversation_inputs(self) -> PurePosixPath:
        """Return the saved exact-file projection, or fail a Resume without it."""
        if self.root is None:
            raise RuntimeError("remote run stage is not open")
        target = self.root / "inputs" / "conversations"
        result = self._ssh(["test", "-d", str(target)])
        if result.returncode:
            raise StateUnavailable(
    ...
    def remove_conversation_inputs(self) -> None:
        """Remove only the exact transcript projection and verify it is gone."""
        if self.root is None:
            raise RuntimeError("remote run stage is not open")
        target = self.root / "inputs" / "conversations"
        script = (
        ...
        result = self._ssh(["python3", "-c", script, str(target)])
        if result.returncode:
            raise StateUnavailable(
                result.stderr.strip() or "could not remove remote conversation inputs"
            )
